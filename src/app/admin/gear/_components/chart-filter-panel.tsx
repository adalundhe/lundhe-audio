...
export type ChartFilterGrouping = "alpha" | "decade";
...
const sortChartFilterOptionsAlphabetically = (options: ChartFilterOption[]) =>
...
export function ChartFilterPanel({
  ...
  return (
    <div className={cn("flex min-h-0 flex-col", className)}>
      ...
      {isFilterInactive ? null : (
        <div className="border-t px-3 py-2 text-xs text-muted-foreground">
          {effectiveSelectedKeys.length}/{options.length} selected
        </div>
      )}
    </div>
  );
