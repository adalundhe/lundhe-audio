"use client";
import { Layout } from "~/components/Layout";
import { Card, CardContent } from "~/components/ui/card";
import type { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Lündhé Audio | Legal | Privacy',
  description: "Lündhé Audio, an Austin based post-tracking mixing, mastering, sound design, and commercial audio studio.",
  icons: [
    {
      rel: "icon",
      url: "/favicon.ico",
    }
  ],
}

export default function Privacy() {
  return (
    <>
      <Layout>
        <Card className="w-full md:w-3/4 h-full rounded-none border-none shadow-none">
          <CardContent className="p-0 w-[100%] h-full flex flex-col items-center">
            <div data-custom-class="body">
              <div>
                <strong>
                  <span style={{ fontSize: 26 }}>
                    <span data-custom-class="title">
                      <b className="block-component" />
                      <b className="question">PRIVACY POLICY</b>
                      <b className="statement-end-if-in-editor" />
                    </span>
                  </span>
                </strong>
              </div>
              <div>
                <br />
              </div>
              <div>
                <span>
                  <strong>
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="subtitle">
                        Last updated{" "}
                        <b className="question">October 28, 2024</b>
                      </span>
                    </span>
                  </strong>
                </span>
              </div>
              <div>
                <br />
              </div>
              <div>
                <br />
              </div>
              <div>
                <br />
              </div>
              <div style={{ lineHeight: "1.5" }}>
                <span>
                  <span>
                    <span data-custom-class="body_text">
                      This Privacy Notice for{" "}
                      <b className="question">
                        Lundhe Audio LLC
                        <b className="block-component" />
                      </b>{" "}
                      (<b className="block-component" />"<strong>we</strong>," "
                      <strong>us</strong>," or "<strong>our</strong>"
                      <b className="statement-end-if-in-editor" />
                    </span>
                    <span data-custom-class="body_text">
                      ), describes how and why we might access, collect, store,
                      use, and/or share (<b className="block-component" />"
                      <strong>process</strong>"
                      <b className="statement-end-if-in-editor" />) your
                      personal information when you use our services (
                      <b className="block-component" />"
                      <strong>Services</strong>"
                      <b className="statement-end-if-in-editor" />
                      ), including when you:
                    </span>
                  </span>
                </span>
                <span style={{ fontSize: 15 }}>
                  <span>
                    <span data-custom-class="body_text">
                      <span style={{  }}>
                        <span data-custom-class="body_text">
                          <b className="block-component" />
                        </span>
                      </span>
                    </span>
                  </span>
                </span>
              </div>
              <ul className="list-disc mx-8 my-4">
                <li data-custom-class="body_text" style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        Visit our website
                        <b className="block-component" /> at{" "}
                        <span>
                          <b className="question">
                            <a
                              target="_blank"
                              data-custom-class="link"
                              href="http://www.lundhe.audio"
                              className="text-cyan-500 hover:underline"
                            >
                              www.lundhe.audio
                            </a>
                          </b>
                        </span>
                        <span style={{ fontSize: 15 }}>
                          <span style={{  }}>
                            <span data-custom-class="body_text">
                              <span style={{ fontSize: 15 }}>
                                <span style={{  }}>
                                  <b className="statement-end-if-in-editor">
                                    , or any website of ours that links to this
                                    Privacy Notice
                                  </b>
                                </span>
                              </span>
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </li>
              </ul>
              <div>
                <b className="block-component">
                  <span style={{ fontSize: 15 }}>
                    <span style={{ fontSize: 15 }}>
                      <span>
                        <span data-custom-class="body_text">
                          <span style={{  }}>
                            <span data-custom-class="body_text">
                              <b className="block-component" />
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </b>
                <div style={{ lineHeight: "1.5" }}>
                  <b className="block-component">
                    <span style={{ fontSize: 15 }} />
                  </b>
                </div>
                <ul className="list-disc mx-8">
                  <li
                    data-custom-class="body_text"
                    style={{ lineHeight: "1.5" }}
                  >
                    <span style={{ fontSize: 15 }}>
                      Use <b className="question">Lundhe Audio</b>.{" "}
                      <b className="question">
                        Mixing, mastering, and other audio services.
                      </b>
                    </span>
                    <b className="statement-end-if-in-editor">
                      <span style={{ fontSize: 15 }} />
                    </b>
                  </li>
                </ul>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span>
                      <span data-custom-class="body_text">
                        <span style={{  }}>
                          <span data-custom-class="body_text">
                            <b className="block-component" />
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <ul className="list-disc mx-8">
                  <li
                    data-custom-class="body_text"
                    style={{ lineHeight: "1.5" }}
                  >
                    <span style={{ fontSize: 15,  }}>
                      <span style={{ fontSize: 15,  }}>
                        <span data-custom-class="body_text">
                          Engage with us in other related ways, including any
                          sales, marketing, or events
                          <span style={{ fontSize: 15 }}>
                            <span style={{  }}>
                              <span data-custom-class="body_text">
                                <span style={{ fontSize: 15 }}>
                                  <span style={{  }}>
                                    <b className="statement-end-if-in-editor" />
                                  </span>
                                </span>
                              </span>
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </li>
                </ul>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span>
                      <span data-custom-class="body_text">
                        <strong>Questions or concerns?&nbsp;</strong>Reading
                        this Privacy Notice will help you understand your
                        privacy rights and choices. We are responsible for
                        making decisions about how your personal information is
                        processed. If you do not agree with our policies and
                        practices, please do not use our Services.
                        <b className="block-component" /> If you still have any
                        questions or concerns, please contact us at{" "}
                        <b className="question">
                          <a
                            target="_blank"
                            data-custom-class="link"
                            href="mailto:adalundhe@lundhe.audio"
                            className="text-cyan-500 hover:underline"
                          >
                            adalundhe@lundhe.audio
                          </a>
                        </b>
                        .
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <strong>
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="heading_1">
                        SUMMARY OF KEY POINTS
                      </span>
                    </span>
                  </strong>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>
                        <em>
                          This summary provides key points from our Privacy
                          Notice, but you can find out more details about any of
                          these topics by clicking the link following each key
                          point or by using our&nbsp;
                        </em>
                      </strong>
                    </span>
                  </span>
                  <a data-custom-class="link" href="#toc">
                    <span style={{ fontSize: 15 }} className="text-cyan-500 hover:underline">
                      <span data-custom-class="body_text">
                        <strong>
                          <em>table of contents</em>
                        </strong>
                      </span>
                    </span>
                  </a>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>
                        <em>
                          &nbsp;below to find the section you are looking for.
                        </em>
                      </strong>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>What personal information do we process?</strong>{" "}
                      When you visit, use, or navigate our Services, we may
                      process personal information depending on how you interact
                      with us and the Services, the choices you make, and the
                      products and features you use. Learn more about&nbsp;
                    </span>
                  </span>
                  <a data-custom-class="link" href="#personalinfo" className="text-cyan-500 hover:underline">
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="body_text">
                        personal information you disclose to us
                      </span>
                    </span>
                  </a>
                  <span data-custom-class="body_text">.</span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>
                        Do we process any sensitive personal information?&nbsp;
                      </strong>
                      Some of the information may be considered{" "}
                      <b className="block-component" />
                      "special" or "sensitive"
                      <b className="statement-end-if-in-editor" /> in certain
                      jurisdictions, for example your racial or ethnic origins,
                      sexual orientation, and religious beliefs.{" "}
                      <b className="block-component" />
                      We do not process sensitive personal information.
                      <b className="else-block" />
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>
                        Do we collect any information from third parties?
                      </strong>{" "}
                      <b className="block-component" />
                      We do not collect any information from third parties.
                      <b className="else-block" />
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>How do we process your information?</strong> We
                      process your information to provide, improve, and
                      administer our Services, communicate with you, for
                      security and fraud prevention, and to comply with law. We
                      may also process your information for other purposes with
                      your consent. We process your information only when we
                      have a valid legal reason to do so. Learn more about&nbsp;
                    </span>
                  </span>
                  <a data-custom-class="link" href="#infouse" className="text-cyan-500 hover:underline">
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="body_text">
                        how we process your information
                      </span>
                    </span>
                  </a>
                  <span data-custom-class="body_text">.</span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>
                        In what situations and with which{" "}
                        <b className="block-component" />
                        parties do we share personal information?
                      </strong>{" "}
                      We may share information in specific situations and with
                      specific <b className="block-component" />
                      third parties. Learn more about&nbsp;
                    </span>
                  </span>
                  <a data-custom-class="link" href="#whoshare" className="text-cyan-500 hover:underline">
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="body_text">
                        when and with whom we share your personal information
                      </span>
                    </span>
                  </a>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      .<b className="block-component" />
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>How do we keep your information safe?</strong> We
                      have adequate <b className="block-component" />
                      organizational
                      <b className="statement-end-if-in-editor" /> and technical
                      processes and procedures in place to protect your personal
                      information. However, no electronic transmission over the
                      internet or information storage technology can be
                      guaranteed to be 100% secure, so we cannot promise or
                      guarantee that hackers, cybercriminals, or other{" "}
                      <b className="block-component" />
                      unauthorized
                      <b className="statement-end-if-in-editor" /> third parties
                      will not be able to defeat our security and improperly
                      collect, access, steal, or modify your information. Learn
                      more about&nbsp;
                    </span>
                  </span>
                  <a data-custom-class="link" href="#infosafe" className="text-cyan-500 hover:underline">
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="body_text">
                        how we keep your information safe
                      </span>
                    </span>
                  </a>
                  <span data-custom-class="body_text">.</span>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <b className="statement-end-if-in-editor" />
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>What are your rights?</strong> Depending on where
                      you are located geographically, the applicable privacy law
                      may mean you have certain rights regarding your personal
                      information. Learn more about&nbsp;
                    </span>
                  </span>
                  <a data-custom-class="link" href="#privacyrights" className="text-cyan-500 hover:underline">
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="body_text">
                        your privacy rights
                      </span>
                    </span>
                  </a>
                  <span data-custom-class="body_text">.</span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>How do you exercise your rights?</strong> The
                      easiest way to exercise your rights is by{" "}
                      <b className="block-component">submitting a&nbsp;</b>
                    </span>
                  </span>
                  <a
                    data-custom-class="link"
                    href="https://app.termly.io/dsar/ae423e99-f627-4e8a-bccb-728f87320370"
                    rel="noopener noreferrer"
                    target="_blank"
                    className="text-cyan-500 hover:underline"
                  >
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="body_text">
                        data subject access request
                      </span>
                    </span>
                  </a>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <b className="block-component" />, or by contacting us. We
                      will consider and act upon any request in accordance with
                      applicable data protection laws.
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      Want to learn more about what we do with any information
                      we collect?&nbsp;
                    </span>
                  </span>
                  <a data-custom-class="link" href="#toc" className="text-cyan-500 hover:underline">
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="body_text">
                        Review the Privacy Notice in full
                      </span>
                    </span>
                  </a>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">.</span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div id="toc" style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span style={{  }}>
                      <span style={{ color: "rgb(0, 0, 0)" }}>
                        <strong>
                          <span data-custom-class="heading_1">
                            TABLE OF CONTENTS
                          </span>
                        </strong>
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <a data-custom-class="link" href="#infocollect" className="text-cyan-500 hover:underline">
                      <span>
                        1. WHAT INFORMATION DO WE COLLECT?
                      </span>
                    </a>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <a data-custom-class="link" href="#infouse" className="text-cyan-500 hover:underline">
                      <span>
                        2. HOW DO WE PROCESS YOUR INFORMATION?
                        <b className="block-component" />
                      </span>
                    </a>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span >
                      <a data-custom-class="link" href="#whoshare" className="text-cyan-500 hover:underline">
                        3. WHEN AND WITH WHOM DO WE SHARE YOUR PERSONAL
                        INFORMATION?
                      </a>
                    </span>
                    <span data-custom-class="body_text">
                      <b className="block-component" />
                      <span style={{  }}>
                        <span style={{  }}>
                          <span data-custom-class="body_text">
                            <span style={{  }}>
                              <b className="block-component" />
                            </span>
                          </span>
                        </span>
                      </span>
                      <b className="block-component" />
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span style={{  }}>
                      <span style={{  }}>
                        <span data-custom-class="body_text">
                          <span style={{  }}>
                            <span style={{  }}>
                              <span style={{  }}>
                                <b className="block-component" />
                              </span>
                            </span>
                            <b className="block-component" />
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <a data-custom-class="link" href="#inforetain" className="text-cyan-500 hover:underline">
                      <span >
                        4. HOW LONG DO WE KEEP YOUR INFORMATION?
                      </span>
                    </a>
                    <span style={{  }}>
                      <span style={{  }}>
                        <span data-custom-class="body_text">
                          <span style={{  }}>
                            <span style={{  }}>
                              <b className="block-component" />
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <a data-custom-class="link" href="#infosafe" className="text-cyan-500 hover:underline">
                      <span>
                        5. HOW DO WE KEEP YOUR INFORMATION SAFE?
                      </span>
                    </a>
                    <span style={{  }}>
                      <span style={{  }}>
                        <span data-custom-class="body_text">
                          <span style={{  }}>
                            <b className="statement-end-if-in-editor" />
                            <b className="block-component" />
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <a data-custom-class="link" href="#infominors" className="text-cyan-500 hover:underline">
                      <span>
                        6. DO WE COLLECT INFORMATION FROM MINORS?
                      </span>
                    </a>
                    <span style={{  }}>
                      <span style={{  }}>
                        <span data-custom-class="body_text">
                          <span style={{  }}>
                            <b className="statement-end-if-in-editor" />
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span>
                      <a data-custom-class="link" href="#privacyrights" className="text-cyan-500 hover:underline">
                        7. WHAT ARE YOUR PRIVACY RIGHTS?
                      </a>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <a data-custom-class="link" href="#DNT" className="text-cyan-500 hover:underline">
                      <span>
                        8. CONTROLS FOR DO-NOT-TRACK FEATURES
                        <b className="block-component" />
                      </span>
                    </a>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <a data-custom-class="link" href="#uslaws" className="text-cyan-500 hover:underline">
                      <span>
                        9. DO UNITED STATES RESIDENTS HAVE SPECIFIC PRIVACY
                        RIGHTS?
                      </span>
                    </a>
                  </span>
                  <b className="block-component">
                    <span style={{ fontSize: 15 }}>
                      <span data-custom-class="body_text" />
                    </span>
                  </b>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <b className="block-component">
                    <span style={{ fontSize: 15 }} />
                  </b>
                  <b className="block-component" />
                  <b className="block-component" />
                  <b className="block-component" />
                  <b className="block-component" />
                  <b className="block-component" />
                  <b className="block-component" />
                  <b className="block-component" />
                  <b className="block-component" />
                  <b className="block-component" />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <a data-custom-class="link" href="#policyupdates" className="text-cyan-500 hover:underline">
                      <span>
                        10. DO WE MAKE UPDATES TO THIS NOTICE?
                      </span>
                    </a>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <a data-custom-class="link" href="#contact" className="text-cyan-500 hover:underline">
                    <span style={{ fontSize: 15 }}>
                      11. HOW CAN YOU CONTACT US ABOUT THIS NOTICE?
                    </span>
                  </a>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <a data-custom-class="link" href="#request" className="text-cyan-500 hover:underline">
                    <span>
                      12. HOW CAN YOU REVIEW, UPDATE, OR DELETE THE DATA WE
                      COLLECT FROM YOU?
                    </span>
                  </a>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div id="infocollect" style={{ lineHeight: "1.5" }}>
                  <span style={{ color: "rgb(0, 0, 0)" }}>
                    <span style={{ color: "rgb(0, 0, 0)", fontSize: 15 }}>
                      <span style={{ fontSize: 15, color: "rgb(0, 0, 0)" }}>
                        <span style={{ fontSize: 15, color: "rgb(0, 0, 0)" }}>
                          <span id="control" style={{ color: "rgb(0, 0, 0)" }}>
                            <strong>
                              <span data-custom-class="heading_1">
                                1. WHAT INFORMATION DO WE COLLECT?
                              </span>
                            </strong>
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div id="personalinfo" style={{ lineHeight: "1.5" }}>
                  <span
                    data-custom-class="heading_2"
                    style={{ color: "rgb(0, 0, 0)" }}
                  >
                    <span style={{ fontSize: 15 }}>
                      <strong>Personal information you disclose to us</strong>
                    </span>
                  </span>
                </div>
                <div>
                  <div>
                    <br />
                  </div>
                  <div style={{ lineHeight: "1.5" }}>
                    <span style={{  }}>
                      <span style={{  fontSize: 15 }}>
                        <span data-custom-class="body_text">
                          <span
                            style={{ fontSize: 15,  }}
                          >
                            <span
                              style={{ fontSize: 15,  }}
                            >
                              <span data-custom-class="body_text">
                                <strong>
                                  <em>In Short:</em>
                                </strong>
                              </span>
                            </span>
                          </span>
                        </span>
                        <span data-custom-class="body_text">
                          <span
                            style={{ fontSize: 15,  }}
                          >
                            <span
                              style={{ fontSize: 15,  }}
                            >
                              <span data-custom-class="body_text">
                                <strong>
                                  <em>&nbsp;</em>
                                </strong>
                                <em>
                                  We collect personal information that you
                                  provide to us.
                                </em>
                              </span>
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </div>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        We collect personal information that you voluntarily
                        provide to us when you{" "}
                        <span style={{ fontSize: 15 }}>
                          <b className="block-component" />
                        </span>
                      </span>
                      <span data-custom-class="body_text">
                        express an interest in obtaining information about us or
                        our products and Services, when you participate in
                        activities on the Services, or otherwise when you
                        contact us.
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <span style={{ fontSize: 15 }}>
                          <b className="block-component" />
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <strong>Personal Information Provided by You.</strong>{" "}
                        The personal information that we collect depends on the
                        context of your interactions with us and the Services,
                        the choices you make, and the products and features you
                        use. The personal information we collect may include the
                        following:
                        <span style={{ fontSize: 15 }}>
                          <span data-custom-class="body_text">
                            <b className="forloop-component" />
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <ul className="list-disc mx-8">
                  <li
                    data-custom-class="body_text"
                    style={{ lineHeight: "1.5" }}
                  >
                    <span style={{ fontSize: 15,  }}>
                      <span style={{ fontSize: 15,  }}>
                        <span data-custom-class="body_text">
                          <span style={{ fontSize: 15 }}>
                            <span data-custom-class="body_text">
                              <b className="question">names</b>
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </li>
                </ul>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <span style={{ fontSize: 15 }}>
                          <span data-custom-class="body_text">
                            <b className="forloop-component" />
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <ul className="list-disc mx-8">
                  <li
                    data-custom-class="body_text"
                    style={{ lineHeight: "1.5" }}
                  >
                    <span style={{ fontSize: 15,  }}>
                      <span style={{ fontSize: 15,  }}>
                        <span data-custom-class="body_text">
                          <span style={{ fontSize: 15 }}>
                            <span data-custom-class="body_text">
                              <b className="question">phone numbers</b>
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </li>
                </ul>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <span style={{ fontSize: 15 }}>
                          <span data-custom-class="body_text">
                            <b className="forloop-component" />
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <ul className="list-disc mx-8">
                  <li
                    data-custom-class="body_text"
                    style={{ lineHeight: "1.5" }}
                  >
                    <span style={{ fontSize: 15,  }}>
                      <span style={{ fontSize: 15,  }}>
                        <span data-custom-class="body_text">
                          <span style={{ fontSize: 15 }}>
                            <span data-custom-class="body_text">
                              <b className="question">email addresses</b>
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </li>
                </ul>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <span style={{ fontSize: 15 }}>
                          <span data-custom-class="body_text">
                            <b className="forloop-component" />
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <ul className="list-disc mx-8">
                  <li
                    data-custom-class="body_text"
                    style={{ lineHeight: "1.5" }}
                  >
                    <span style={{ fontSize: 15,  }}>
                      <span style={{ fontSize: 15,  }}>
                        <span data-custom-class="body_text">
                          <span style={{ fontSize: 15 }}>
                            <span data-custom-class="body_text">
                              <b className="question">contact preferences</b>
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </li>
                </ul>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <span style={{ fontSize: 15 }}>
                          <span data-custom-class="body_text">
                            <b className="forloop-component" />
                          </span>
                          <span data-custom-class="body_text">
                            <b className="statement-end-if-in-editor" />
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <div id="sensitiveinfo" style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <strong>Sensitive Information.</strong>{" "}
                      <b className="block-component" />
                      We do not process sensitive information.
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <b className="else-block" />
                    </span>
                  </span>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <span style={{ fontSize: 15 }}>
                          <span data-custom-class="body_text">
                            <b className="block-component">
                              <b className="block-component" />
                            </b>
                          </span>
                        </span>
                      </span>
                    </span>
                    <b className="block-component" />
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        All personal information that you provide to us must be
                        true, complete, and accurate, and you must notify us of
                        any changes to such personal information.
                      </span>
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <b className="block-component" />
                      </span>
                    </span>
                  </span>
                  <b className="block-component">
                    <span style={{ fontSize: 15 }} />
                  </b>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <span
                          style={{  fontSize: 15 }}
                        >
                          <span data-custom-class="body_text">
                            <span
                              style={{  fontSize: 15 }}
                            >
                              <span data-custom-class="body_text">
                                <b className="statement-end-if-in-editor">
                                  <b className="block-component" />
                                </b>
                              </span>
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                  <span style={{ fontSize: 15 }}>
                    <span data-custom-class="body_text">
                      <b className="block-component" />
                    </span>
                  </span>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div id="infouse" style={{ lineHeight: "1.5" }}>
                  <span style={{  }}>
                    <span style={{  fontSize: 15 }}>
                      <span style={{ fontSize: 15,  }}>
                        <span
                          style={{ fontSize: 15,  }}
                        >
                          <span id="control" style={{ color: "rgb(0, 0, 0)" }}>
                            <strong>
                              <span data-custom-class="heading_1">
                                2. HOW DO WE PROCESS YOUR INFORMATION?
                              </span>
                            </strong>
                          </span>
                        </span>
                      </span>
                    </span>
                  </span>
                </div>
                <div>
                  <div style={{ lineHeight: "1.5" }}>
                    <br />
                  </div>
                  <div style={{ lineHeight: "1.5" }}>
                    <span style={{  }}>
                      <span style={{  fontSize: 15 }}>
                        <span data-custom-class="body_text">
                          <span
                            style={{ fontSize: 15,  }}
                          >
                            <span
                              style={{ fontSize: 15,  }}
                            >
                              <span data-custom-class="body_text">
                                <strong>
                                  <em>In Short:&nbsp;</em>
                                </strong>
                                <em>
                                  We process your information to provide,
                                  improve, and administer our Services,
                                  communicate with you, for security and fraud
                                  prevention, and to comply with law. We may
                                  also process your information for other
                                  purposes with your consent.
                                </em>
                              </span>
                            </span>
                          </span>
                        </span>
                      </span>
                    </span>
                  </div>
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <br />
                </div>
                <div style={{ lineHeight: "1.5" }}>
                  <span style={{ fontSize: 15,  }}>
                    <span style={{ fontSize: 15,  }}>
                      <span data-custom-class="body_text">
                        <strong>
                          We process your personal information for a variety of
                          reasons, depending on how you interact with our
                          Services, including:
                        </strong>
                        <b className="block-component" />
                      </span>
                    </span>
                  </span>
                  <div style={{ lineHeight: "1.5" }}>
                    <span style={{ fontSize: 15,  }}>
                      <span style={{ fontSize: 15,  }}>
                        <span data-custom-class="body_text">
                          <b className="block-component" />
                        </span>
                      </span>
                    </span>
                  </div>
                  <ul className="list-disc mx-8">
                    <li
                      data-custom-class="body_text"
                      style={{ lineHeight: "1.5" }}
                    >
                      <span style={{ fontSize: 15,  }}>
                        <span
                          style={{ fontSize: 15,  }}
                        >
                          <span data-custom-class="body_text">
                            <strong>
                              To deliver and facilitate delivery of services to
                              the user.&nbsp;
                            </strong>
                            We may process your information to provide you with
                            the requested service.
                            <span
                              style={{ fontSize: 15,  }}
                            >
                              <span
                                style={{
                                  fontSize: 15,
                                  
                                }}
                              >
                                <span data-custom-class="body_text">
                                  <span
                                    style={{
                                      fontSize: 15,
                                      
                                    }}
                                  >
                                    <span
                                      style={{
                                        fontSize: 15,
                                        
                                      }}
                                    >
                                      <span data-custom-class="body_text">
                                        <span style={{ fontSize: 15 }}>
                                          <span
                                            style={{  }}
                                          >
                                            <span data-custom-class="body_text">
                                              <span style={{ fontSize: 15 }}>
                                                <span
                                                  style={{
                                                    
                                                  }}
                                                >
                                                  <span data-custom-class="body_text">
                                                    <b className="statement-end-if-in-editor" />
                                                  </span>
                                                </span>
                                              </span>
                                            </span>
                                          </span>
                                        </span>
                                      </span>
                                    </span>
                                  </span>
                                </span>
                              </span>
                            </span>
                          </span>
                        </span>
                      </span>
                    </li>
                  </ul>
                  <div style={{ lineHeight: "1.5" }}>
                    <span style={{ fontSize: 15,  }}>
                      <span style={{ fontSize: 15,  }}>
                        <span data-custom-class="body_text">
                          <b className="block-component" />
                        </span>
                      </span>
                    </span>
                    <div style={{ lineHeight: "1.5" }}>
                      <span style={{ fontSize: 15,  }}>
                        <span
                          style={{ fontSize: 15,  }}
                        >
                          <span data-custom-class="body_text">
                            <b className="block-component" />
                          </span>
                        </span>
                      </span>
                    </div>
                    <ul className="list-disc mx-8">
                      <li
                        data-custom-class="body_text"
                        style={{ lineHeight: "1.5" }}
                      >
                        <span
                          style={{ fontSize: 15,  }}
                        >
                          <span
                            style={{ fontSize: 15,  }}
                          >
                            <span data-custom-class="body_text">
                              <strong>
                                To respond to user inquiries/offer support to
                                users.&nbsp;
                              </strong>
                              We may process your information to respond to your
                              inquiries and solve any potential issues you might
                              have with the requested service.
                              <span
                                style={{
                                  fontSize: 15,
                                  
                                }}
                              >
                                <span
                                  style={{
                                    fontSize: 15,
                                    
                                  }}
                                >
                                  <span data-custom-class="body_text">
                                    <span style={{ fontSize: 15 }}>
                                      <span
                                        style={{  }}
                                      >
                                        <span data-custom-class="body_text">
                                          <span style={{ fontSize: 15 }}>
                                            <span
                                              style={{
                                                
                                              }}
                                            >
                                              <span data-custom-class="body_text">
                                                <b className="statement-end-if-in-editor" />
                                              </span>
                                            </span>
                                          </span>
                                        </span>
                                      </span>
                                    </span>
                                  </span>
                                </span>
                              </span>
                            </span>
                          </span>
                        </span>
                      </li>
                    </ul>
                    <div style={{ lineHeight: "1.5" }}>
                      <span style={{ fontSize: 15,  }}>
                        <span
                          style={{ fontSize: 15,  }}
                        >
                          <span data-custom-class="body_text">
                            <b className="block-component" />
                          </span>
                        </span>
                      </span>
                      <div style={{ lineHeight: "1.5" }}>
                        <b className="block-component">
                          <span style={{ fontSize: 15 }} />
                        </b>
                      </div>
                      <ul className="list-disc mx-8">
                        <li
                          data-custom-class="body_text"
                          style={{ lineHeight: "1.5" }}
                        >
                          <span
                            style={{ fontSize: 15,  }}
                          >
                            <span
                              style={{ fontSize: 15,  }}
                            >
                              <span data-custom-class="body_text">
                                <strong>
                                  To send administrative information to
                                  you.&nbsp;
                                </strong>
                                We may process your information to send you
                                details about our products and services, changes
                                to our terms and policies, and other similar
                                information.
                                <span
                                  style={{
                                    fontSize: 15,
                                    
                                  }}
                                >
                                  <span
                                    style={{
                                      fontSize: 15,
                                      
                                    }}
                                  >
                                    <span data-custom-class="body_text">
                                      <b className="statement-end-if-in-editor" />
                                    </span>
                                  </span>
                                </span>
                              </span>
                            </span>
                          </span>
                        </li>
                      </ul>
                      <div style={{ lineHeight: "1.5" }}>
                        <b className="block-component">
                          <span style={{ fontSize: 15 }} />
                        </b>
                        <div style={{ lineHeight: "1.5" }}>
                          <b className="block-component">
                            <span style={{ fontSize: 15 }} />
                          </b>
                        </div>
                        <ul className="list-disc mx-8">
                          <li
                            data-custom-class="body_text"
                            style={{ lineHeight: "1.5" }}
                          >
                            <span
                              style={{ fontSize: 15,  }}
                            >
                              <span
                                style={{
                                  fontSize: 15,
                                  
                                }}
                              >
                                <span data-custom-class="body_text">
                                  <strong>
                                    To <b className="block-component" />
                                    fulfill
                                    <b className="statement-end-if-in-editor" />{" "}
                                    and manage your orders.&nbsp;
                                  </strong>
                                  We may process your information to{" "}
                                  <b className="block-component" />
                                  fulfill
                                  <b className="statement-end-if-in-editor" />{" "}
                                  and manage your orders, payments, returns, and
                                  exchanges made through the Services.
                                  <span
                                    style={{
                                      fontSize: 15,
                                      
                                    }}
                                  >
                                    <span
                                      style={{
                                        fontSize: 15,
                                        
                                      }}
                                    >
                                      <span data-custom-class="body_text">
                                        <span
                                          style={{
                                            fontSize: 15,
                                            
                                          }}
                                        >
                                          <span
                                            style={{
                                              fontSize: 15,
                                              
                                            }}
                                          >
                                            <span data-custom-class="body_text">
                                              <span
                                                style={{
                                                  fontSize: 15,
                                                  
                                                }}
                                              >
                                                <span
                                                  style={{
                                                    fontSize: 15,
                                                    
                                                  }}
                                                >
                                                  <span data-custom-class="body_text">
                                                    <b className="statement-end-if-in-editor" />
                                                  </span>
                                                </span>
                                              </span>
                                            </span>
                                          </span>
                                        </span>
                                      </span>
                                    </span>
                                  </span>
                                </span>
                              </span>
                            </span>
                          </li>
                        </ul>
                        <div style={{ lineHeight: "1.5" }}>
                          <b className="block-component">
                            <span style={{ fontSize: 15 }}>
                              <span data-custom-class="body_text" />
                            </span>
                          </b>
                          <p style={{ fontSize: 15, lineHeight: "1.5" }}>
                            <b className="block-component">
                              <span style={{ fontSize: 15 }} />
                            </b>
                          </p>
                          <ul className="list-disc mx-8">
                            <li
                              data-custom-class="body_text"
                              style={{ lineHeight: "1.5" }}
                            >
                              <span style={{ fontSize: 15 }}>
                                <span style={{  }}>
                                  <span style={{  }}>
                                    <span data-custom-class="body_text">
                                      <strong>
                                        To enable user-to-user
                                        communications.&nbsp;
                                      </strong>
                                      We may process your information if you
                                      choose to use any of our offerings that
                                      allow for communication with another user.
                                      <span
                                        style={{  }}
                                      >
                                        <span
                                          style={{  }}
                                        >
                                          <span data-custom-class="body_text">
                                            <span
                                              style={{
                                                
                                              }}
                                            >
                                              <span data-custom-class="body_text">
                                                <span
                                                  style={{
                                                    
                                                  }}
                                                >
                                                  <span data-custom-class="body_text">
                                                    <b className="statement-end-if-in-editor" />
                                                  </span>
                                                </span>
                                              </span>
                                            </span>
                                          </span>
                                        </span>
                                      </span>
                                    </span>
                                  </span>
                                </span>
                              </span>
                            </li>
                          </ul>
                          <p style={{ fontSize: 15, lineHeight: "1.5" }}>
                            <b className="block-component">
                              <span style={{ fontSize: 15 }} />
                            </b>
                          </p>
                          <p style={{ fontSize: 15, lineHeight: "1.5" }}>
                            <b className="block-component" />
                          </p>
                          <ul className="list-disc mx-8">
                            <li
                              data-custom-class="body_text"
                              style={{ lineHeight: "1.5" }}
                            >
                              <span style={{ fontSize: 15 }}>
                                <span style={{  }}>
                                  <span style={{  }}>
                                    <span data-custom-class="body_text">
                                      <strong>
                                        To request feedback.&nbsp;
                                      </strong>
                                      We may process your information when
                                      necessary to request feedback and to
                                      contact you about your use of our
                                      Services.
                                      <span
                                        style={{  }}
                                      >
                                        <span
                                          style={{  }}
                                        >
                                          <span data-custom-class="body_text">
                                            <span
                                              style={{
                                                
                                              }}
                                            >
                                              <span data-custom-class="body_text">
                                                <span
                                                  style={{
                                                    
                                                  }}
                                                >
                                                  <span data-custom-class="body_text">
                                                    <b className="statement-end-if-in-editor" />
                                                  </span>
                                                </span>
                                              </span>
                                            </span>
                                          </span>
                                        </span>
                                      </span>
                                    </span>
                                  </span>
                                </span>
                              </span>
                            </li>
                          </ul>
                          <p style={{ fontSize: 15, lineHeight: "1.5" }}>
                            <b className="block-component" />
                          </p>
                          <div style={{ lineHeight: "1.5" }}>
                            <b className="block-component">
                              <span style={{ fontSize: 15 }}>
                                <span data-custom-class="body_text" />
                              </span>
                            </b>
                            <div style={{ lineHeight: "1.5" }}>
                              <b className="block-component">
                                <span style={{ fontSize: 15 }} />
                              </b>
                              <div style={{ lineHeight: "1.5" }}>
                                <b className="block-component">
                                  <span style={{ fontSize: 15 }} />
                                </b>
                                <div style={{ lineHeight: "1.5" }}>
                                  <span style={{ fontSize: 15 }}>
                                    <b className="block-component">
                                      <span data-custom-class="body_text" />
                                    </b>
                                  </span>
                                  <div style={{ lineHeight: "1.5" }}>
                                    <b className="block-component">
                                      <span style={{ fontSize: 15 }}>
                                        <span data-custom-class="body_text" />
                                      </span>
                                    </b>
                                    <div style={{ lineHeight: "1.5" }}>
                                      <b className="block-component">
                                        <span style={{ fontSize: 15 }}>
                                          <span data-custom-class="body_text" />
                                        </span>
                                      </b>
                                      <div style={{ lineHeight: "1.5" }}>
                                        <b className="block-component">
                                          <span style={{ fontSize: 15 }}>
                                            <span data-custom-class="body_text" />
                                          </span>
                                        </b>
                                        <div style={{ lineHeight: "1.5" }}>
                                          <b className="block-component">
                                            <span style={{ fontSize: 15 }}>
                                              <span data-custom-class="body_text" />
                                            </span>
                                          </b>
                                          <div style={{ lineHeight: "1.5" }}>
                                            <b className="block-component">
                                              <span style={{ fontSize: 15 }}>
                                                <span data-custom-class="body_text" />
                                              </span>
                                            </b>
                                            <div style={{ lineHeight: "1.5" }}>
                                              <b className="block-component">
                                                <span style={{ fontSize: 15 }}>
                                                  <span data-custom-class="body_text" />
                                                </span>
                                              </b>
                                              <div
                                                style={{ lineHeight: "1.5" }}
                                              >
                                                <b className="block-component">
                                                  <span
                                                    style={{ fontSize: 15 }}
                                                  >
                                                    <span data-custom-class="body_text" />
                                                  </span>
                                                </b>
                                                <div
                                                  style={{ lineHeight: "1.5" }}
                                                >
                                                  <b className="block-component">
                                                    <span
                                                      style={{ fontSize: 15 }}
                                                    >
                                                      <span data-custom-class="body_text" />
                                                    </span>
                                                  </b>
                                                  <div
                                                    style={{
                                                      lineHeight: "1.5",
                                                    }}
                                                  >
                                                    <b className="block-component">
                                                      <span
                                                        style={{ fontSize: 15 }}
                                                      >
                                                        <span data-custom-class="body_text" />
                                                      </span>
                                                    </b>
                                                    <div
                                                      style={{
                                                        lineHeight: "1.5",
                                                      }}
                                                    >
                                                      <b className="block-component">
                                                        <span
                                                          style={{
                                                            fontSize: 15,
                                                          }}
                                                        >
                                                          <span data-custom-class="body_text" />
                                                        </span>
                                                      </b>
                                                      <div
                                                        style={{
                                                          lineHeight: "1.5",
                                                        }}
                                                      >
                                                        <b className="block-component">
                                                          <span
                                                            style={{
                                                              fontSize: 15,
                                                            }}
                                                          >
                                                            <span data-custom-class="body_text" />
                                                          </span>
                                                        </b>
                                                        <div
                                                          style={{
                                                            lineHeight: "1.5",
                                                          }}
                                                        >
                                                          <b className="block-component">
                                                            <span
                                                              style={{
                                                                fontSize: 15,
                                                              }}
                                                            >
                                                              <span data-custom-class="body_text" />
                                                            </span>
                                                          </b>
                                                          <b className="block-component">
                                                            <span
                                                              style={{
                                                                fontSize: 15,
                                                              }}
                                                            >
                                                              <span data-custom-class="body_text" />
                                                            </span>
                                                          </b>
                                                          <b className="block-component">
                                                            <span
                                                              style={{
                                                                fontSize: 15,
                                                              }}
                                                            >
                                                              <span data-custom-class="body_text" />
                                                            </span>
                                                          </b>
                                                          <b className="block-component">
                                                            <span
                                                              style={{
                                                                fontSize: 15,
                                                              }}
                                                            >
                                                              <span data-custom-class="body_text" />
                                                            </span>
                                                          </b>
                                                        </div>
                                                        <div
                                                          style={{
                                                            lineHeight: "1.5",
                                                          }}
                                                        >
                                                          <br />
                                                        </div>
                                                        <div
                                                          id="whoshare"
                                                          style={{
                                                            lineHeight: "1.5",
                                                          }}
                                                        >
                                                          <span
                                                            style={{
                                                              color:
                                                                "rgb(127, 127, 127)",
                                                            }}
                                                          >
                                                            <span
                                                              style={{
                                                                color:
                                                                  "",
                                                                fontSize: 15,
                                                              }}
                                                            >
                                                              <span
                                                                style={{
                                                                  fontSize: 15,
                                                                  color:
                                                                    "",
                                                                }}
                                                              >
                                                                <span
                                                                  style={{
                                                                    fontSize: 15,
                                                                    color:
                                                                      "",
                                                                  }}
                                                                >
                                                                  <span
                                                                    id="control"
                                                                    style={{
                                                                      color:
                                                                        "rgb(0, 0, 0)",
                                                                    }}
                                                                  >
                                                                    <strong>
                                                                      <span data-custom-class="heading_1">
                                                                        3. WHEN
                                                                        AND WITH
                                                                        WHOM DO
                                                                        WE SHARE
                                                                        YOUR
                                                                        PERSONAL
                                                                        INFORMATION?
                                                                      </span>
                                                                    </strong>
                                                                  </span>
                                                                </span>
                                                              </span>
                                                            </span>
                                                          </span>
                                                        </div>
                                                        <div
                                                          style={{
                                                            lineHeight: "1.5",
                                                          }}
                                                        >
                                                          <br />
                                                        </div>
                                                        <div
                                                          style={{
                                                            lineHeight: "1.5",
                                                          }}
                                                        >
                                                          <span
                                                            style={{
                                                              fontSize: 15,
                                                              color:
                                                                "",
                                                            }}
                                                          >
                                                            <span
                                                              style={{
                                                                fontSize: 15,
                                                                color:
                                                                  "",
                                                              }}
                                                            >
                                                              <span data-custom-class="body_text">
                                                                <strong>
                                                                  <em>
                                                                    In Short:
                                                                  </em>
                                                                </strong>
                                                                <em>
                                                                  &nbsp;We may
                                                                  share
                                                                  information in
                                                                  specific
                                                                  situations
                                                                  described in
                                                                  this section
                                                                  and/or with
                                                                  the following{" "}
                                                                  <b className="block-component" />
                                                                  third parties.
                                                                </em>
                                                              </span>
                                                            </span>
                                                          </span>
                                                        </div>
                                                        <div
                                                          style={{
                                                            lineHeight: "1.5",
                                                          }}
                                                        >
                                                          <span
                                                            style={{
                                                              fontSize: 15,
                                                              color:
                                                                "",
                                                            }}
                                                          >
                                                            <span
                                                              style={{
                                                                fontSize: 15,
                                                                color:
                                                                  "",
                                                              }}
                                                            >
                                                              <span data-custom-class="body_text">
                                                                <b className="block-component" />
                                                              </span>
                                                            </span>
                                                          </span>
                                                        </div>
                                                        <div
                                                          style={{
                                                            lineHeight: "1.5",
                                                          }}
                                                        >
                                                          <br />
                                                        </div>
                                                        <div
                                                          style={{
                                                            lineHeight: "1.5",
                                                          }}
                                                        >
                                                          <span
                                                            style={{
                                                              fontSize: 15,
                                                            }}
                                                          >
                                                            <span data-custom-class="body_text">
                                                              We{" "}
                                                              <b className="block-component" />
                                                              may need to share
                                                              your personal
                                                              information in the
                                                              following
                                                              situations:
                                                            </span>
                                                          </span>
                                                        </div>
                                                        <ul className="list-disc mx-8">
                                                          <li
                                                            data-custom-class="body_text"
                                                            style={{
                                                              lineHeight: "1.5",
                                                            }}
                                                          >
                                                            <span
                                                              style={{
                                                                fontSize: 15,
                                                              }}
                                                            >
                                                              <span data-custom-class="body_text">
                                                                <strong>
                                                                  Business
                                                                  Transfers.
                                                                </strong>{" "}
                                                                We may share or
                                                                transfer your
                                                                information in
                                                                connection with,
                                                                or during
                                                                negotiations of,
                                                                any merger, sale
                                                                of company
                                                                assets,
                                                                financing, or
                                                                acquisition of
                                                                all or a portion
                                                                of our business
                                                                to another
                                                                company.
                                                              </span>
                                                            </span>
                                                          </li>
                                                        </ul>
                                                        <div
                                                          style={{
                                                            lineHeight: "1.5",
                                                          }}
                                                        >
                                                          <span
                                                            style={{
                                                              fontSize: 15,
                                                            }}
                                                          >
                                                            <span data-custom-class="body_text">
                                                              <b className="block-component" />
                                                            </span>
                                                          </span>
                                                          <div
                                                            style={{
                                                              lineHeight: "1.5",
                                                            }}
                                                          >
                                                            <span
                                                              style={{
                                                                fontSize: 15,
                                                              }}
                                                            >
                                                              <b className="block-component">
                                                                <span data-custom-class="body_text" />
                                                              </b>
                                                            </span>
                                                            <div
                                                              style={{
                                                                lineHeight:
                                                                  "1.5",
                                                              }}
                                                            >
                                                              <b className="block-component">
                                                                <span
                                                                  style={{
                                                                    fontSize: 15,
                                                                  }}
                                                                >
                                                                  <span data-custom-class="body_text" />
                                                                </span>
                                                              </b>
                                                              <div
                                                                style={{
                                                                  lineHeight:
                                                                    "1.5",
                                                                }}
                                                              >
                                                                <b className="block-component">
                                                                  <span
                                                                    style={{
                                                                      fontSize: 15,
                                                                    }}
                                                                  >
                                                                    <span data-custom-class="body_text" />
                                                                  </span>
                                                                </b>
                                                                <div
                                                                  style={{
                                                                    lineHeight:
                                                                      "1.5",
                                                                  }}
                                                                >
                                                                  <b className="block-component">
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <span data-custom-class="body_text" />
                                                                    </span>
                                                                  </b>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <b className="block-component">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text" />
                                                                      </span>
                                                                    </b>
                                                                    <span
                                                                      style={{
                                                                        color:
                                                                          "",
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span
                                                                          style={{
                                                                            color:
                                                                              "",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <b className="block-component">
                                                                                <span data-custom-class="heading_1">
                                                                                  <b className="block-component" />
                                                                                </span>
                                                                              </b>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <b className="block-component" />
                                                                    </span>
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <span
                                                                            style={{
                                                                              color:
                                                                                "",
                                                                              fontSize: 15,
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                color:
                                                                                  "",
                                                                                fontSize: 15,
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  color:
                                                                                    "",
                                                                                  fontSize: 15,
                                                                                }}
                                                                              >
                                                                                <span
                                                                                  style={{
                                                                                    fontSize: 15,
                                                                                  }}
                                                                                >
                                                                                  <span
                                                                                    style={{
                                                                                      color:
                                                                                        "",
                                                                                    }}
                                                                                  >
                                                                                    <span
                                                                                      style={{
                                                                                        fontSize: 15,
                                                                                      }}
                                                                                    >
                                                                                      <span
                                                                                        style={{
                                                                                          color:
                                                                                            "",
                                                                                        }}
                                                                                      >
                                                                                        <span data-custom-class="body_text">
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                        <b className="block-component">
                                                                                          <span data-custom-class="body_text">
                                                                                            <b className="block-component" />
                                                                                          </span>
                                                                                        </b>
                                                                                      </span>
                                                                                    </span>
                                                                                  </span>
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    id="inforetain"
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        color:
                                                                          "rgb(127, 127, 127)",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          color:
                                                                            "",
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span
                                                                          style={{
                                                                            fontSize: 15,
                                                                            color:
                                                                              "",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              id="control"
                                                                              style={{
                                                                                color:
                                                                                  "rgb(0, 0, 0)",
                                                                              }}
                                                                            >
                                                                              <strong>
                                                                                <span data-custom-class="heading_1">
                                                                                  4.
                                                                                  HOW
                                                                                  LONG
                                                                                  DO
                                                                                  WE
                                                                                  KEEP
                                                                                  YOUR
                                                                                  INFORMATION?
                                                                                </span>
                                                                              </strong>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <strong>
                                                                            <em>
                                                                              In
                                                                              Short:&nbsp;
                                                                            </em>
                                                                          </strong>
                                                                          <em>
                                                                            We
                                                                            keep
                                                                            your
                                                                            information
                                                                            for
                                                                            as
                                                                            long
                                                                            as
                                                                            necessary
                                                                            to{" "}
                                                                            <b className="block-component" />
                                                                            fulfill
                                                                            <b className="statement-end-if-in-editor" />{" "}
                                                                            the
                                                                            purposes
                                                                            outlined
                                                                            in
                                                                            this
                                                                            Privacy
                                                                            Notice
                                                                            unless
                                                                            otherwise
                                                                            required
                                                                            by
                                                                            law.
                                                                          </em>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          We
                                                                          will
                                                                          only
                                                                          keep
                                                                          your
                                                                          personal
                                                                          information
                                                                          for as
                                                                          long
                                                                          as it
                                                                          is
                                                                          necessary
                                                                          for
                                                                          the
                                                                          purposes
                                                                          set
                                                                          out in
                                                                          this
                                                                          Privacy
                                                                          Notice,
                                                                          unless
                                                                          a
                                                                          longer
                                                                          retention
                                                                          period
                                                                          is
                                                                          required
                                                                          or
                                                                          permitted
                                                                          by law
                                                                          (such
                                                                          as
                                                                          tax,
                                                                          accounting,
                                                                          or
                                                                          other
                                                                          legal
                                                                          requirements).
                                                                          <b className="block-component" />{" "}
                                                                          No
                                                                          purpose
                                                                          in
                                                                          this
                                                                          notice
                                                                          will
                                                                          require
                                                                          us
                                                                          keeping
                                                                          your
                                                                          personal
                                                                          information
                                                                          for
                                                                          longer
                                                                          than{" "}
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component" />
                                                                                <b className="question">
                                                                                  90
                                                                                  days
                                                                                </b>
                                                                                <b className="statement-end-if-in-editor" />
                                                                              </span>
                                                                            </span>
                                                                          </span>
                                                                          .
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          When
                                                                          we
                                                                          have
                                                                          no
                                                                          ongoing
                                                                          legitimate
                                                                          business
                                                                          need
                                                                          to
                                                                          process
                                                                          your
                                                                          personal
                                                                          information,
                                                                          we
                                                                          will
                                                                          either
                                                                          delete
                                                                          or{" "}
                                                                          <b className="block-component" />
                                                                          anonymize
                                                                          <b className="statement-end-if-in-editor" />{" "}
                                                                          such
                                                                          information,
                                                                          or, if
                                                                          this
                                                                          is not
                                                                          possible
                                                                          (for
                                                                          example,
                                                                          because
                                                                          your
                                                                          personal
                                                                          information
                                                                          has
                                                                          been
                                                                          stored
                                                                          in
                                                                          backup
                                                                          archives),
                                                                          then
                                                                          we
                                                                          will
                                                                          securely
                                                                          store
                                                                          your
                                                                          personal
                                                                          information
                                                                          and
                                                                          isolate
                                                                          it
                                                                          from
                                                                          any
                                                                          further
                                                                          processing
                                                                          until
                                                                          deletion
                                                                          is
                                                                          possible.
                                                                          <span
                                                                            style={{
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <b className="block-component" />
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    id="infosafe"
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        color:
                                                                          "rgb(127, 127, 127)",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          color:
                                                                            "",
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span
                                                                          style={{
                                                                            fontSize: 15,
                                                                            color:
                                                                              "",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              id="control"
                                                                              style={{
                                                                                color:
                                                                                  "rgb(0, 0, 0)",
                                                                              }}
                                                                            >
                                                                              <strong>
                                                                                <span data-custom-class="heading_1">
                                                                                  5.
                                                                                  HOW
                                                                                  DO
                                                                                  WE
                                                                                  KEEP
                                                                                  YOUR
                                                                                  INFORMATION
                                                                                  SAFE?
                                                                                </span>
                                                                              </strong>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <strong>
                                                                            <em>
                                                                              In
                                                                              Short:&nbsp;
                                                                            </em>
                                                                          </strong>
                                                                          <em>
                                                                            We
                                                                            aim
                                                                            to
                                                                            protect
                                                                            your
                                                                            personal
                                                                            information
                                                                            through
                                                                            a
                                                                            system
                                                                            of{" "}
                                                                            <b className="block-component" />
                                                                            organizational
                                                                            <b className="statement-end-if-in-editor" />{" "}
                                                                            and
                                                                            technical
                                                                            security
                                                                            measures.
                                                                          </em>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          We
                                                                          have
                                                                          implemented
                                                                          appropriate
                                                                          and
                                                                          reasonable
                                                                          technical
                                                                          and{" "}
                                                                          <b className="block-component" />
                                                                          organizational
                                                                          <b className="statement-end-if-in-editor" />{" "}
                                                                          security
                                                                          measures
                                                                          designed
                                                                          to
                                                                          protect
                                                                          the
                                                                          security
                                                                          of any
                                                                          personal
                                                                          information
                                                                          we
                                                                          process.
                                                                          However,
                                                                          despite
                                                                          our
                                                                          safeguards
                                                                          and
                                                                          efforts
                                                                          to
                                                                          secure
                                                                          your
                                                                          information,
                                                                          no
                                                                          electronic
                                                                          transmission
                                                                          over
                                                                          the
                                                                          Internet
                                                                          or
                                                                          information
                                                                          storage
                                                                          technology
                                                                          can be
                                                                          guaranteed
                                                                          to be
                                                                          100%
                                                                          secure,
                                                                          so we
                                                                          cannot
                                                                          promise
                                                                          or
                                                                          guarantee
                                                                          that
                                                                          hackers,
                                                                          cybercriminals,
                                                                          or
                                                                          other{" "}
                                                                          <b className="block-component" />
                                                                          unauthorized
                                                                          <b className="statement-end-if-in-editor" />{" "}
                                                                          third
                                                                          parties
                                                                          will
                                                                          not be
                                                                          able
                                                                          to
                                                                          defeat
                                                                          our
                                                                          security
                                                                          and
                                                                          improperly
                                                                          collect,
                                                                          access,
                                                                          steal,
                                                                          or
                                                                          modify
                                                                          your
                                                                          information.
                                                                          Although
                                                                          we
                                                                          will
                                                                          do our
                                                                          best
                                                                          to
                                                                          protect
                                                                          your
                                                                          personal
                                                                          information,
                                                                          transmission
                                                                          of
                                                                          personal
                                                                          information
                                                                          to and
                                                                          from
                                                                          our
                                                                          Services
                                                                          is at
                                                                          your
                                                                          own
                                                                          risk.
                                                                          You
                                                                          should
                                                                          only
                                                                          access
                                                                          the
                                                                          Services
                                                                          within
                                                                          a
                                                                          secure
                                                                          environment.
                                                                          <span
                                                                            style={{
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <b className="statement-end-if-in-editor" />
                                                                          </span>
                                                                          <span
                                                                            style={{
                                                                              color:
                                                                                "",
                                                                              fontSize: 15,
                                                                            }}
                                                                          >
                                                                            <span data-custom-class="body_text">
                                                                              <b className="block-component" />
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    id="infominors"
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        color:
                                                                          "rgb(127, 127, 127)",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          color:
                                                                            "",
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span
                                                                          style={{
                                                                            fontSize: 15,
                                                                            color:
                                                                              "",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              id="control"
                                                                              style={{
                                                                                color:
                                                                                  "rgb(0, 0, 0)",
                                                                              }}
                                                                            >
                                                                              <strong>
                                                                                <span data-custom-class="heading_1">
                                                                                  6.
                                                                                  DO
                                                                                  WE
                                                                                  COLLECT
                                                                                  INFORMATION
                                                                                  FROM
                                                                                  MINORS?
                                                                                </span>
                                                                              </strong>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <strong>
                                                                            <em>
                                                                              In
                                                                              Short:
                                                                            </em>
                                                                          </strong>
                                                                          <em>
                                                                            &nbsp;We
                                                                            do
                                                                            not
                                                                            knowingly
                                                                            collect
                                                                            data
                                                                            from
                                                                            or
                                                                            market
                                                                            to{" "}
                                                                            <b className="block-component" />
                                                                            children
                                                                            under
                                                                            18
                                                                            years
                                                                            of
                                                                            age
                                                                            <b className="else-block" />
                                                                            .
                                                                          </em>
                                                                          <b className="block-component" />
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          We do
                                                                          not
                                                                          knowingly
                                                                          collect,
                                                                          solicit
                                                                          data
                                                                          from,
                                                                          or
                                                                          market
                                                                          to
                                                                          children
                                                                          under
                                                                          18
                                                                          years
                                                                          of
                                                                          age,
                                                                          nor do
                                                                          we
                                                                          knowingly
                                                                          sell
                                                                          such
                                                                          personal
                                                                          information.
                                                                          By
                                                                          using
                                                                          the
                                                                          Services,
                                                                          you
                                                                          represent
                                                                          that
                                                                          you
                                                                          are at
                                                                          least
                                                                          18 or
                                                                          that
                                                                          you
                                                                          are
                                                                          the
                                                                          parent
                                                                          or
                                                                          guardian
                                                                          of
                                                                          such a
                                                                          minor
                                                                          and
                                                                          consent
                                                                          to
                                                                          such
                                                                          minor
                                                                          dependent’s
                                                                          use of
                                                                          the
                                                                          Services.
                                                                          If we
                                                                          learn
                                                                          that
                                                                          personal
                                                                          information
                                                                          from
                                                                          users
                                                                          less
                                                                          than
                                                                          18
                                                                          years
                                                                          of age
                                                                          has
                                                                          been
                                                                          collected,
                                                                          we
                                                                          will
                                                                          deactivate
                                                                          the
                                                                          account
                                                                          and
                                                                          take
                                                                          reasonable
                                                                          measures
                                                                          to
                                                                          promptly
                                                                          delete
                                                                          such
                                                                          data
                                                                          from
                                                                          our
                                                                          records.
                                                                          If you
                                                                          become
                                                                          aware
                                                                          of any
                                                                          data
                                                                          we may
                                                                          have
                                                                          collected
                                                                          from
                                                                          children
                                                                          under
                                                                          age
                                                                          18,
                                                                          please
                                                                          contact
                                                                          us at{" "}
                                                                          <span
                                                                            style={{
                                                                              color:
                                                                                "",
                                                                              fontSize: 15,
                                                                            }}
                                                                          >
                                                                            <span data-custom-class="body_text">
                                                                              <b className="block-component" />
                                                                              <b className="question">
                                                                                <a
                                                                                  target="_blank"
                                                                                  data-custom-class="link"
                                                                                  href="mailto:adalundhe@lundhe.audio"
                                                                                  className="text-cyan-500 hover:underline"
                                                                                >
                                                                                  adalundhe@lundhe.audio
                                                                                </a>
                                                                              </b>
                                                                              <b className="else-block" />
                                                                            </span>
                                                                          </span>
                                                                          .
                                                                        </span>
                                                                        <span data-custom-class="body_text">
                                                                          <b className="else-block">
                                                                            <b className="block-component" />
                                                                          </b>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    id="privacyrights"
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        color:
                                                                          "rgb(127, 127, 127)",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          color:
                                                                            "",
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span
                                                                          style={{
                                                                            fontSize: 15,
                                                                            color:
                                                                              "",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              id="control"
                                                                              style={{
                                                                                color:
                                                                                  "rgb(0, 0, 0)",
                                                                              }}
                                                                            >
                                                                              <strong>
                                                                                <span data-custom-class="heading_1">
                                                                                  7.
                                                                                  WHAT
                                                                                  ARE
                                                                                  YOUR
                                                                                  PRIVACY
                                                                                  RIGHTS?
                                                                                </span>
                                                                              </strong>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <strong>
                                                                            <em>
                                                                              In
                                                                              Short:
                                                                            </em>
                                                                          </strong>
                                                                          <em>
                                                                            &nbsp;
                                                                            <span
                                                                              style={{
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <em>
                                                                                    <b className="block-component" />
                                                                                  </em>
                                                                                </span>
                                                                              </span>
                                                                              &nbsp;
                                                                            </span>
                                                                            You
                                                                            may
                                                                            review,
                                                                            change,
                                                                            or
                                                                            terminate
                                                                            your
                                                                            account
                                                                            at
                                                                            any
                                                                            time,
                                                                            depending
                                                                            on
                                                                            your
                                                                            country,
                                                                            province,
                                                                            or
                                                                            state
                                                                            of
                                                                            residence.
                                                                          </em>
                                                                          <span
                                                                            style={{
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                              }}
                                                                            >
                                                                              <b className="block-component">
                                                                                <b className="block-component" />
                                                                              </b>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    id="withdrawconsent"
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <strong>
                                                                            <u>
                                                                              Withdrawing
                                                                              your
                                                                              consent:
                                                                            </u>
                                                                          </strong>{" "}
                                                                          If we
                                                                          are
                                                                          relying
                                                                          on
                                                                          your
                                                                          consent
                                                                          to
                                                                          process
                                                                          your
                                                                          personal
                                                                          information,
                                                                          <b className="block-component" />{" "}
                                                                          which
                                                                          may be
                                                                          express
                                                                          and/or
                                                                          implied
                                                                          consent
                                                                          depending
                                                                          on the
                                                                          applicable
                                                                          law,
                                                                          <b className="statement-end-if-in-editor" />{" "}
                                                                          you
                                                                          have
                                                                          the
                                                                          right
                                                                          to
                                                                          withdraw
                                                                          your
                                                                          consent
                                                                          at any
                                                                          time.
                                                                          You
                                                                          can
                                                                          withdraw
                                                                          your
                                                                          consent
                                                                          at any
                                                                          time
                                                                          by
                                                                          contacting
                                                                          us by
                                                                          using
                                                                          the
                                                                          contact
                                                                          details
                                                                          provided
                                                                          in the
                                                                          section{" "}
                                                                          <b className="block-component" />
                                                                          "
                                                                          <b className="statement-end-if-in-editor" />
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                    <a
                                                                      data-custom-class="link"
                                                                      href="#contact"
                                                                      className="text-cyan-500 hover:underline"
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span
                                                                          style={{
                                                                            fontSize: 15,
                                                                          }}
                                                                        >
                                                                          <span data-custom-class="body_text">
                                                                            HOW
                                                                            CAN
                                                                            YOU
                                                                            CONTACT
                                                                            US
                                                                            ABOUT
                                                                            THIS
                                                                            NOTICE?
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </a>
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <b className="block-component" />
                                                                          "
                                                                          <b className="statement-end-if-in-editor" />{" "}
                                                                          below
                                                                          <b className="block-component" />
                                                                          .
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <span data-custom-class="body_text">
                                                                        However,
                                                                        please
                                                                        note
                                                                        that
                                                                        this
                                                                        will not
                                                                        affect
                                                                        the
                                                                        lawfulness
                                                                        of the
                                                                        processing
                                                                        before
                                                                        its
                                                                        withdrawal
                                                                        nor,
                                                                        <b className="block-component" />{" "}
                                                                        when
                                                                        applicable
                                                                        law
                                                                        allows,
                                                                        <b className="statement-end-if-in-editor" />{" "}
                                                                        will it
                                                                        affect
                                                                        the
                                                                        processing
                                                                        of your
                                                                        personal
                                                                        information
                                                                        conducted
                                                                        in
                                                                        reliance
                                                                        on
                                                                        lawful
                                                                        processing
                                                                        grounds
                                                                        other
                                                                        than
                                                                        consent.
                                                                        <b className="block-component" />
                                                                      </span>
                                                                    </span>
                                                                    <b className="block-component">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text" />
                                                                      </span>
                                                                    </b>
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                }}
                                                                              >
                                                                                <span
                                                                                  style={{
                                                                                    color:
                                                                                      "",
                                                                                  }}
                                                                                >
                                                                                  <span data-custom-class="body_text">
                                                                                    <span
                                                                                      style={{
                                                                                        fontSize: 15,
                                                                                      }}
                                                                                    >
                                                                                      <span
                                                                                        style={{
                                                                                          color:
                                                                                            "",
                                                                                        }}
                                                                                      >
                                                                                        <b className="block-component" />
                                                                                      </span>
                                                                                    </span>
                                                                                  </span>
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                    <b className="block-component">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text" />
                                                                      </span>
                                                                    </b>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span data-custom-class="body_text">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        If you
                                                                        have
                                                                        questions
                                                                        or
                                                                        comments
                                                                        about
                                                                        your
                                                                        privacy
                                                                        rights,
                                                                        you may
                                                                        email us
                                                                        at{" "}
                                                                        <b className="question">
                                                                          <a
                                                                            target="_blank"
                                                                            data-custom-class="link"
                                                                            href="mailto:adalundhe@lundhe.audio"
                                                                            className="text-cyan-500 hover:underline"
                                                                          >
                                                                            adalundhe@lundhe.audio
                                                                          </a>
                                                                        </b>
                                                                        .
                                                                      </span>
                                                                    </span>
                                                                    <b className="statement-end-if-in-editor">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text" />
                                                                      </span>
                                                                    </b>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    id="DNT"
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        color:
                                                                          "rgb(127, 127, 127)",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          color:
                                                                            "",
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span
                                                                          style={{
                                                                            fontSize: 15,
                                                                            color:
                                                                              "",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              id="control"
                                                                              style={{
                                                                                color:
                                                                                  "rgb(0, 0, 0)",
                                                                              }}
                                                                            >
                                                                              <strong>
                                                                                <span data-custom-class="heading_1">
                                                                                  8.
                                                                                  CONTROLS
                                                                                  FOR
                                                                                  DO-NOT-TRACK
                                                                                  FEATURES
                                                                                </span>
                                                                              </strong>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          Most
                                                                          web
                                                                          browsers
                                                                          and
                                                                          some
                                                                          mobile
                                                                          operating
                                                                          systems
                                                                          and
                                                                          mobile
                                                                          applications
                                                                          include
                                                                          a
                                                                          Do-Not-Track
                                                                          (
                                                                          <b className="block-component" />
                                                                          "DNT"
                                                                          <b className="statement-end-if-in-editor" />
                                                                          )
                                                                          feature
                                                                          or
                                                                          setting
                                                                          you
                                                                          can
                                                                          activate
                                                                          to
                                                                          signal
                                                                          your
                                                                          privacy
                                                                          preference
                                                                          not to
                                                                          have
                                                                          data
                                                                          about
                                                                          your
                                                                          online
                                                                          browsing
                                                                          activities
                                                                          monitored
                                                                          and
                                                                          collected.
                                                                          At
                                                                          this
                                                                          stage,
                                                                          no
                                                                          uniform
                                                                          technology
                                                                          standard
                                                                          for{" "}
                                                                          <b className="block-component" />
                                                                          recognizing
                                                                          <b className="statement-end-if-in-editor" />{" "}
                                                                          and
                                                                          implementing
                                                                          DNT
                                                                          signals
                                                                          has
                                                                          been{" "}
                                                                          <b className="block-component" />
                                                                          finalized
                                                                          <b className="statement-end-if-in-editor" />
                                                                          . As
                                                                          such,
                                                                          we do
                                                                          not
                                                                          currently
                                                                          respond
                                                                          to DNT
                                                                          browser
                                                                          signals
                                                                          or any
                                                                          other
                                                                          mechanism
                                                                          that
                                                                          automatically
                                                                          communicates
                                                                          your
                                                                          choice
                                                                          not to
                                                                          be
                                                                          tracked
                                                                          online.
                                                                          If a
                                                                          standard
                                                                          for
                                                                          online
                                                                          tracking
                                                                          is
                                                                          adopted
                                                                          that
                                                                          we
                                                                          must
                                                                          follow
                                                                          in the
                                                                          future,
                                                                          we
                                                                          will
                                                                          inform
                                                                          you
                                                                          about
                                                                          that
                                                                          practice
                                                                          in a
                                                                          revised
                                                                          version
                                                                          of
                                                                          this
                                                                          Privacy
                                                                          Notice.
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                    <b className="block-component">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      />
                                                                    </b>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <br />
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <span data-custom-class="body_text">
                                                                        California
                                                                        law
                                                                        requires
                                                                        us to
                                                                        let you
                                                                        know how
                                                                        we
                                                                        respond
                                                                        to web
                                                                        browser
                                                                        DNT
                                                                        signals.
                                                                        Because
                                                                        there
                                                                        currently
                                                                        is not
                                                                        an
                                                                        industry
                                                                        or legal
                                                                        standard
                                                                        for{" "}
                                                                        <b className="block-component" />
                                                                        recognizing
                                                                        <b className="statement-end-if-in-editor" />{" "}
                                                                        or{" "}
                                                                        <b className="block-component" />
                                                                        honoring
                                                                        <b className="statement-end-if-in-editor" />{" "}
                                                                        DNT
                                                                        signals,
                                                                        we do
                                                                        not
                                                                        respond
                                                                        to them
                                                                        at this
                                                                        time.
                                                                      </span>
                                                                    </span>
                                                                    <b className="statement-end-if-in-editor">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      />
                                                                    </b>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <b className="block-component" />
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    id="uslaws"
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        color:
                                                                          "rgb(127, 127, 127)",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          color:
                                                                            "",
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        <span
                                                                          style={{
                                                                            fontSize: 15,
                                                                            color:
                                                                              "",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              id="control"
                                                                              style={{
                                                                                color:
                                                                                  "rgb(0, 0, 0)",
                                                                              }}
                                                                            >
                                                                              <strong>
                                                                                <span data-custom-class="heading_1">
                                                                                  9.
                                                                                  DO
                                                                                  UNITED
                                                                                  STATES
                                                                                  RESIDENTS
                                                                                  HAVE
                                                                                  SPECIFIC
                                                                                  PRIVACY
                                                                                  RIGHTS?
                                                                                </span>
                                                                              </strong>
                                                                            </span>
                                                                          </span>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <strong>
                                                                            <em>
                                                                              In
                                                                              Short:&nbsp;
                                                                            </em>
                                                                          </strong>
                                                                          <em>
                                                                            If
                                                                            you
                                                                            are
                                                                            a
                                                                            resident
                                                                            of
                                                                            <b className="block-component" />{" "}
                                                                            California,
                                                                            Colorado,
                                                                            Connecticut,
                                                                            Delaware,
                                                                            Florida,
                                                                            Indiana,
                                                                            Iowa,
                                                                            Kentucky,
                                                                            Minnesota,
                                                                            Montana,
                                                                            Nebraska,
                                                                            New
                                                                            Hampshire,
                                                                            New
                                                                            Jersey,
                                                                            Oregon,
                                                                            Tennessee,
                                                                            Texas,
                                                                            Utah,
                                                                            or
                                                                            Virginia
                                                                            <b className="else-block" />
                                                                            ,
                                                                            you
                                                                            may
                                                                            have
                                                                            the
                                                                            right
                                                                            to
                                                                            request
                                                                            access
                                                                            to
                                                                            and
                                                                            receive
                                                                            details
                                                                            about
                                                                            the
                                                                            personal
                                                                            information
                                                                            we
                                                                            maintain
                                                                            about
                                                                            you
                                                                            and
                                                                            how
                                                                            we
                                                                            have
                                                                            processed
                                                                            it,
                                                                            correct
                                                                            inaccuracies,
                                                                            get
                                                                            a
                                                                            copy
                                                                            of,
                                                                            or
                                                                            delete
                                                                            your
                                                                            personal
                                                                            information.
                                                                            You
                                                                            may
                                                                            also
                                                                            have
                                                                            the
                                                                            right
                                                                            to
                                                                            withdraw
                                                                            your
                                                                            consent
                                                                            to
                                                                            our
                                                                            processing
                                                                            of
                                                                            your
                                                                            personal
                                                                            information.
                                                                            These
                                                                            rights
                                                                            may
                                                                            be
                                                                            limited
                                                                            in
                                                                            some
                                                                            circumstances
                                                                            by
                                                                            applicable
                                                                            law.
                                                                            More
                                                                            information
                                                                            is
                                                                            provided
                                                                            below.
                                                                          </em>
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <strong>
                                                                          <span data-custom-class="heading_2">
                                                                            Categories
                                                                            of
                                                                            Personal
                                                                            Information
                                                                            We
                                                                            Collect
                                                                          </span>
                                                                        </strong>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                        color:
                                                                          "",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                          color:
                                                                            "",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          We
                                                                          have
                                                                          collected
                                                                          the
                                                                          following
                                                                          categories
                                                                          of
                                                                          personal
                                                                          information
                                                                          in the
                                                                          past
                                                                          twelve
                                                                          (12)
                                                                          months:
                                                                        </span>
                                                                      </span>
                                                                    </span>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <table
                                                                    style={{
                                                                      width:
                                                                        "100%",
                                                                    }}
                                                                  >
                                                                    <tbody>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span data-custom-class="body_text">
                                                                                <strong>
                                                                                  Category
                                                                                </strong>
                                                                              </span>
                                                                            </span>
                                                                          </span>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span data-custom-class="body_text">
                                                                                <strong>
                                                                                  Examples
                                                                                </strong>
                                                                              </span>
                                                                            </span>
                                                                          </span>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                            textAlign:
                                                                              "center",
                                                                          }}
                                                                        >
                                                                          <span
                                                                            style={{
                                                                              fontSize: 15,
                                                                              color:
                                                                                "",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span data-custom-class="body_text">
                                                                                <strong>
                                                                                  Collected
                                                                                </strong>
                                                                              </span>
                                                                            </span>
                                                                          </span>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  A.
                                                                                  Identifiers
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Contact
                                                                                  details,
                                                                                  such
                                                                                  as
                                                                                  real
                                                                                  name,
                                                                                  alias,
                                                                                  postal
                                                                                  address,
                                                                                  telephone
                                                                                  or
                                                                                  mobile
                                                                                  contact
                                                                                  number,
                                                                                  unique
                                                                                  personal
                                                                                  identifier,
                                                                                  online
                                                                                  identifier,
                                                                                  Internet
                                                                                  Protocol
                                                                                  address,
                                                                                  email
                                                                                  address,
                                                                                  and
                                                                                  account
                                                                                  name
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            textAlign:
                                                                              "center",
                                                                            verticalAlign:
                                                                              "middle",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component">
                                                                                    <b className="block-component" />
                                                                                  </b>
                                                                                  YES
                                                                                  <b className="else-block">
                                                                                    <b className="block-component" />
                                                                                  </b>
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                    </tbody>
                                                                  </table>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <b className="block-component" />
                                                                  </div>
                                                                  <table
                                                                    style={{
                                                                      width:
                                                                        "100%",
                                                                    }}
                                                                  >
                                                                    <tbody>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  B.
                                                                                  Personal
                                                                                  information
                                                                                  as
                                                                                  defined
                                                                                  in
                                                                                  the
                                                                                  California
                                                                                  Customer
                                                                                  Records
                                                                                  statute
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Name,
                                                                                  contact
                                                                                  information,
                                                                                  education,
                                                                                  employment,
                                                                                  employment
                                                                                  history,
                                                                                  and
                                                                                  financial
                                                                                  information
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="forloop-component">
                                                                                    <b className="block-component">
                                                                                      <b className="block-component" />
                                                                                      YES
                                                                                      <b className="block-component" />
                                                                                    </b>
                                                                                  </b>
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                    </tbody>
                                                                  </table>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <b className="block-component" />
                                                                  </div>
                                                                  <table
                                                                    style={{
                                                                      width:
                                                                        "100%",
                                                                    }}
                                                                  >
                                                                    <tbody>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                  C
                                                                                  <b className="else-block" />

                                                                                  .
                                                                                  Protected
                                                                                  classification
                                                                                  characteristics
                                                                                  under
                                                                                  state
                                                                                  or
                                                                                  federal
                                                                                  law
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Gender,
                                                                                  age,
                                                                                  date
                                                                                  of
                                                                                  birth,
                                                                                  race
                                                                                  and
                                                                                  ethnicity,
                                                                                  national
                                                                                  origin,
                                                                                  marital
                                                                                  status,
                                                                                  and
                                                                                  other
                                                                                  demographic
                                                                                  data
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <b className="forloop-component">
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component" />
                                                                                <b className="block-component" />
                                                                                NO
                                                                                <b className="statement-end-if-in-editor" />
                                                                                <b className="block-component" />
                                                                              </span>
                                                                            </b>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                  D
                                                                                  <b className="else-block" />

                                                                                  .
                                                                                  Commercial
                                                                                  information
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Transaction
                                                                                  information,
                                                                                  purchase
                                                                                  history,
                                                                                  financial
                                                                                  details,
                                                                                  and
                                                                                  payment
                                                                                  information
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <b className="forloop-component">
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component" />
                                                                                <b className="block-component" />
                                                                                NO
                                                                                <b className="statement-end-if-in-editor">
                                                                                  <b className="block-component" />
                                                                                </b>
                                                                              </span>
                                                                            </b>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                  E
                                                                                  <b className="else-block" />

                                                                                  .
                                                                                  Biometric
                                                                                  information
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Fingerprints
                                                                                  and
                                                                                  voiceprints
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <b className="forloop-component">
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component">
                                                                                  <b className="block-component">
                                                                                    NO
                                                                                  </b>
                                                                                  <b className="statement-end-if-in-editor" />
                                                                                  <b className="block-component" />
                                                                                </b>
                                                                              </span>
                                                                            </b>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                  F
                                                                                  <b className="else-block" />

                                                                                  .
                                                                                  Internet
                                                                                  or
                                                                                  other
                                                                                  similar
                                                                                  network
                                                                                  activity
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Browsing
                                                                                  history,
                                                                                  search
                                                                                  history,
                                                                                  online{" "}
                                                                                  <b className="block-component" />
                                                                                  behavior
                                                                                  <b className="statement-end-if-in-editor" />

                                                                                  ,
                                                                                  interest
                                                                                  data,
                                                                                  and
                                                                                  interactions
                                                                                  with
                                                                                  our
                                                                                  and
                                                                                  other
                                                                                  websites,
                                                                                  applications,
                                                                                  systems,
                                                                                  and
                                                                                  advertisements
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <b className="forloop-component">
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component" />
                                                                                <b className="block-component" />
                                                                                NO
                                                                                <b className="statement-end-if-in-editor" />
                                                                                <b className="block-component" />
                                                                              </span>
                                                                            </b>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                  G
                                                                                  <b className="else-block" />

                                                                                  .
                                                                                  Geolocation
                                                                                  data
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Device
                                                                                  location
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <b className="forloop-component">
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component" />
                                                                                <b className="block-component" />
                                                                                NO
                                                                                <b className="statement-end-if-in-editor" />
                                                                                <b className="block-component" />
                                                                              </span>
                                                                            </b>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                  H
                                                                                  <b className="else-block" />

                                                                                  .
                                                                                  Audio,
                                                                                  electronic,
                                                                                  sensory,
                                                                                  or
                                                                                  similar
                                                                                  information
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Images
                                                                                  and
                                                                                  audio,
                                                                                  video
                                                                                  or
                                                                                  call
                                                                                  recordings
                                                                                  created
                                                                                  in
                                                                                  connection
                                                                                  with
                                                                                  our
                                                                                  business
                                                                                  activities
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <b className="forloop-component">
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component" />
                                                                                <b className="block-component" />
                                                                                NO
                                                                                <b className="statement-end-if-in-editor" />
                                                                                <b className="block-component" />
                                                                              </span>
                                                                            </b>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "33.8274%",
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                  I
                                                                                  <b className="else-block" />

                                                                                  .
                                                                                  Professional
                                                                                  or
                                                                                  employment-related
                                                                                  information
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "51.4385%",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Business
                                                                                  contact
                                                                                  details
                                                                                  in
                                                                                  order
                                                                                  to
                                                                                  provide
                                                                                  you
                                                                                  our
                                                                                  Services
                                                                                  at
                                                                                  a
                                                                                  business
                                                                                  level
                                                                                  or
                                                                                  job
                                                                                  title,
                                                                                  work
                                                                                  history,
                                                                                  and
                                                                                  professional
                                                                                  qualifications
                                                                                  if
                                                                                  you
                                                                                  apply
                                                                                  for
                                                                                  a
                                                                                  job
                                                                                  with
                                                                                  us
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            width:
                                                                              "14.9084%",
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <b className="forloop-component">
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component" />
                                                                                <b className="block-component" />
                                                                                NO
                                                                                <b className="statement-end-if-in-editor" />
                                                                                <b className="block-component" />
                                                                              </span>
                                                                            </b>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                            width:
                                                                              "33.8274%",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                  J
                                                                                  <b className="else-block" />

                                                                                  .
                                                                                  Education
                                                                                  Information
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                            width:
                                                                              "51.4385%",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Student
                                                                                  records
                                                                                  and
                                                                                  directory
                                                                                  information
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                            width:
                                                                              "14.9084%",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <b className="forloop-component">
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component" />
                                                                                <b className="block-component" />
                                                                                NO
                                                                                <b className="statement-end-if-in-editor" />
                                                                                <b className="block-component" />
                                                                              </span>
                                                                            </b>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            borderWidth: 1,
                                                                            borderColor:
                                                                              "black",
                                                                            borderStyle:
                                                                              "solid",
                                                                            width:
                                                                              "33.8274%",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                  K
                                                                                  <b className="else-block" />

                                                                                  .
                                                                                  Inferences
                                                                                  drawn
                                                                                  from
                                                                                  collected
                                                                                  personal
                                                                                  information
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            borderBottom:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            width:
                                                                              "51.4385%",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span
                                                                              style={{
                                                                                fontSize: 15,
                                                                                color:
                                                                                  "",
                                                                              }}
                                                                            >
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  Inferences
                                                                                  drawn
                                                                                  from
                                                                                  any
                                                                                  of
                                                                                  the
                                                                                  collected
                                                                                  personal
                                                                                  information
                                                                                  listed
                                                                                  above
                                                                                  to
                                                                                  create
                                                                                  a
                                                                                  profile
                                                                                  or
                                                                                  summary
                                                                                  about,
                                                                                  for
                                                                                  example,
                                                                                  an
                                                                                  individual’s
                                                                                  preferences
                                                                                  and
                                                                                  characteristics
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            textAlign:
                                                                              "center",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderBottom:
                                                                              "1px solid black",
                                                                            borderTop:
                                                                              "1px solid black",
                                                                            width:
                                                                              "14.9084%",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span data-custom-class="body_text">
                                                                              <b className="block-component" />
                                                                              NO
                                                                              <span
                                                                                style={{
                                                                                  fontSize: 15,
                                                                                  color:
                                                                                    "",
                                                                                }}
                                                                              >
                                                                                <span
                                                                                  style={{
                                                                                    fontSize: 15,
                                                                                    color:
                                                                                      "",
                                                                                  }}
                                                                                >
                                                                                  <span data-custom-class="body_text">
                                                                                    <span
                                                                                      style={{
                                                                                        color:
                                                                                          "",
                                                                                      }}
                                                                                    >
                                                                                      <span data-custom-class="body_text">
                                                                                        <b className="statement-end-if-in-editor" />
                                                                                      </span>
                                                                                    </span>
                                                                                  </span>
                                                                                </span>
                                                                              </span>
                                                                            </span>
                                                                          </div>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                      <tr>
                                                                        <td
                                                                          style={{
                                                                            borderLeft:
                                                                              "1px solid black",
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderBottom:
                                                                              "1px solid black",
                                                                            lineHeight:
                                                                              "1.5",
                                                                          }}
                                                                        >
                                                                          <span data-custom-class="body_text" style={{
                                                                              color:
                                                                                "",
                                                                          }}>
                                                                            <b className="block-component" />
                                                                            L
                                                                            <b className="else-block" />
                                                                            .
                                                                            Sensitive
                                                                            personal
                                                                            Information
                                                                          </span>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderBottom:
                                                                              "1px solid black",
                                                                            lineHeight:
                                                                              "1.5",
                                                                          }}
                                                                        >
                                                                          <b className="block-component">
                                                                            <span data-custom-class="body_text" />
                                                                          </b>
                                                                        </td>
                                                                        <td
                                                                          style={{
                                                                            borderRight:
                                                                              "1px solid black",
                                                                            borderBottom:
                                                                              "1px solid black",
                                                                          }}
                                                                        >
                                                                          <div
                                                                            data-empty="true"
                                                                            style={{
                                                                              textAlign:
                                                                                "center",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                          <div
                                                                            data-custom-class="body_text"
                                                                            data-empty="true"
                                                                            style={{
                                                                              textAlign:
                                                                                "center",
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <b className="block-component">
                                                                              <span data-custom-class="body_text" />
                                                                            </b>
                                                                            NO
                                                                            <b className="statement-end-if-in-editor">
                                                                              <span data-custom-class="body_text" />
                                                                            </b>
                                                                          </div>
                                                                          <div
                                                                            data-empty="true"
                                                                            style={{
                                                                              textAlign:
                                                                                "center",
                                                                            }}
                                                                          >
                                                                            <br />
                                                                          </div>
                                                                        </td>
                                                                      </tr>
                                                                    </tbody>
                                                                  </table>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <b className="block-component">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      />
                                                                    </b>
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <br />
                                                                  </div>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <span data-custom-class="body_text">
                                                                        We may
                                                                        also
                                                                        collect
                                                                        other
                                                                        personal
                                                                        information
                                                                        outside
                                                                        of these
                                                                        categories
                                                                        through
                                                                        instances
                                                                        where
                                                                        you
                                                                        interact
                                                                        with us
                                                                        in
                                                                        person,
                                                                        online,
                                                                        or by
                                                                        phone or
                                                                        mail in
                                                                        the
                                                                        context
                                                                        of:
                                                                      </span>
                                                                      <b className="block-component" />
                                                                    </span>
                                                                  </div>
                                                                  <ul className="list-disc mx-8">
                                                                    <li
                                                                      data-custom-class="body_text"
                                                                      style={{
                                                                        lineHeight:
                                                                          "1.5",
                                                                      }}
                                                                    >
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        Receiving
                                                                        help
                                                                        through
                                                                        our
                                                                        customer
                                                                        support
                                                                        channels;
                                                                        <b className="statement-end-if-in-editor" />
                                                                      </span>
                                                                    </li>
                                                                  </ul>
                                                                  <div>
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <b className="block-component" />
                                                                    </span>
                                                                  </div>
                                                                  <ul className="list-disc mx-8">
                                                                    <li data-custom-class="body_text">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        Participation
                                                                        in
                                                                        customer
                                                                        surveys
                                                                        or
                                                                        contests;
                                                                        and
                                                                        <b className="statement-end-if-in-editor" />
                                                                      </span>
                                                                    </li>
                                                                  </ul>
                                                                  <div>
                                                                    <span
                                                                      style={{
                                                                        fontSize: 15,
                                                                      }}
                                                                    >
                                                                      <b className="block-component" />
                                                                    </span>
                                                                  </div>
                                                                  <ul className="list-disc mx-8">
                                                                    <li data-custom-class="body_text">
                                                                      <span
                                                                        style={{
                                                                          fontSize: 15,
                                                                        }}
                                                                      >
                                                                        Facilitation
                                                                        in the
                                                                        delivery
                                                                        of our
                                                                        Services
                                                                        and to
                                                                        respond
                                                                        to your
                                                                        inquiries.
                                                                      </span>
                                                                      <b className="statement-end-if-in-editor">
                                                                        <span
                                                                          style={{
                                                                            fontSize: 15,
                                                                          }}
                                                                        />
                                                                      </b>
                                                                    </li>
                                                                  </ul>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <b className="block-component">
                                                                      <span data-custom-class="body_text" />
                                                                    </b>
                                                                    <span data-custom-class="body_text">
                                                                      We will
                                                                      use and
                                                                      retain the
                                                                      collected
                                                                      personal
                                                                      information
                                                                      as needed
                                                                      to provide
                                                                      the
                                                                      Services
                                                                      or for:
                                                                      <b className="block-component" />
                                                                    </span>
                                                                  </div>
                                                                  <ul className="list-disc mx-8">
                                                                    <li
                                                                      data-custom-class="body_text"
                                                                      style={{
                                                                        lineHeight:
                                                                          "1.5",
                                                                      }}
                                                                    >
                                                                      <span data-custom-class="body_text" style={{
                                                                              color:
                                                                                "",
                                                                      }}>
                                                                        Category
                                                                        A -{" "}
                                                                        <b className="question">
                                                                          6
                                                                          months
                                                                        </b>
                                                                        <b className="statement-end-if-in-editor" />
                                                                      </span>
                                                                    </li>
                                                                  </ul>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span data-custom-class="body_text">
                                                                      <b className="block-component">
                                                                        <b className="block-component" />
                                                                      </b>
                                                                    </span>
                                                                  </div>
                                                                  <ul className="list-disc mx-8">
                                                                    <li
                                                                      data-custom-class="body_text"
                                                                      style={{
                                                                        lineHeight:
                                                                          "1.5",
                                                                      }}
                                                                    >
                                                                      <span data-custom-class="body_text" style={{
                                                                              color:
                                                                                "",
                                                                      }}>
                                                                        Category
                                                                        B -{" "}
                                                                        <b className="question">
                                                                          6
                                                                          months
                                                                        </b>
                                                                        <b className="statement-end-if-in-editor" />
                                                                      </span>
                                                                    </li>
                                                                  </ul>
                                                                  <div
                                                                    style={{
                                                                      lineHeight:
                                                                        "1.5",
                                                                    }}
                                                                  >
                                                                    <span data-custom-class="body_text">
                                                                      <b className="block-component" />
                                                                    </span>
                                                                    <div
                                                                      style={{
                                                                        lineHeight:
                                                                          "1.5",
                                                                      }}
                                                                    >
                                                                      <span data-custom-class="body_text">
                                                                        <b className="block-component" />
                                                                      </span>
                                                                      <div
                                                                        style={{
                                                                          lineHeight:
                                                                            "1.5",
                                                                        }}
                                                                      >
                                                                        <span data-custom-class="body_text">
                                                                          <b className="block-component" />
                                                                        </span>
                                                                        <div
                                                                          style={{
                                                                            lineHeight:
                                                                              "1.5",
                                                                          }}
                                                                        >
                                                                          <span data-custom-class="body_text">
                                                                            <b className="block-component" />
                                                                          </span>
                                                                          <div
                                                                            style={{
                                                                              lineHeight:
                                                                                "1.5",
                                                                            }}
                                                                          >
                                                                            <span data-custom-class="body_text">
                                                                              <b className="block-component" />
                                                                            </span>
                                                                            <div
                                                                              style={{
                                                                                lineHeight:
                                                                                  "1.5",
                                                                              }}
                                                                            >
                                                                              <span data-custom-class="body_text">
                                                                                <b className="block-component" />
                                                                              </span>
                                                                              <div
                                                                                style={{
                                                                                  lineHeight:
                                                                                    "1.5",
                                                                                }}
                                                                              >
                                                                                <span data-custom-class="body_text">
                                                                                  <b className="block-component" />
                                                                                </span>
                                                                                <div
                                                                                  style={{
                                                                                    lineHeight:
                                                                                      "1.5",
                                                                                  }}
                                                                                >
                                                                                  <span data-custom-class="body_text">
                                                                                    <b className="block-component" />
                                                                                  </span>
                                                                                  <div
                                                                                    style={{
                                                                                      lineHeight:
                                                                                        "1.5",
                                                                                    }}
                                                                                  >
                                                                                    <span data-custom-class="body_text">
                                                                                      <b className="block-component" />
                                                                                    </span>
                                                                                    <div
                                                                                      style={{
                                                                                        lineHeight:
                                                                                          "1.5",
                                                                                      }}
                                                                                    >
                                                                                      <span data-custom-class="body_text">
                                                                                        <b className="block-component" />
                                                                                      </span>
                                                                                      <b className="statement-end-if-in-editor">
                                                                                        <span data-custom-class="body_text" />
                                                                                      </b>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <strong>
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="heading_2">
                                                                                              Sources
                                                                                              of
                                                                                              Personal
                                                                                              Information
                                                                                            </span>
                                                                                          </span>
                                                                                        </strong>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            Learn
                                                                                            more
                                                                                            about
                                                                                            the
                                                                                            sources
                                                                                            of
                                                                                            personal
                                                                                            information
                                                                                            we
                                                                                            collect
                                                                                            in{" "}
                                                                                            <b className="block-component" />
                                                                                            "
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </span>
                                                                                        <span data-custom-class="body_text">
                                                                                          <a
                                                                                            data-custom-class="link"
                                                                                            href="#infocollect"
                                                                                            className="text-cyan-500 hover:underline"
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            >
                                                                                              WHAT
                                                                                              INFORMATION
                                                                                              DO
                                                                                              WE
                                                                                              COLLECT?
                                                                                            </span>
                                                                                          </a>
                                                                                        </span>
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            <b className="block-component" />

                                                                                            "
                                                                                          </span>
                                                                                          <b className="statement-end-if-in-editor">
                                                                                            <span data-custom-class="body_text" />
                                                                                          </b>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <strong>
                                                                                              <span data-custom-class="heading_2">
                                                                                                How
                                                                                                We
                                                                                                Use
                                                                                                and
                                                                                                Share
                                                                                                Personal
                                                                                                Information
                                                                                              </span>
                                                                                            </strong>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                          Learn
                                                                                          more
                                                                                          about
                                                                                          how
                                                                                          we
                                                                                          use
                                                                                          your
                                                                                          personal
                                                                                          information
                                                                                          in
                                                                                          the
                                                                                          section,{" "}
                                                                                          <b className="block-component" />
                                                                                          "
                                                                                          <b className="statement-end-if-in-editor" />
                                                                                        </span>
                                                                                        <a
                                                                                          data-custom-class="link"
                                                                                          href="#infouse"
                                                                                          className="text-cyan-500 hover:underline"
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            HOW
                                                                                            DO
                                                                                            WE
                                                                                            PROCESS
                                                                                            YOUR
                                                                                            INFORMATION?
                                                                                          </span>
                                                                                        </a>
                                                                                        <span
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />

                                                                                          "
                                                                                        </span>
                                                                                        <b className="statement-end-if-in-editor">
                                                                                          <span
                                                                                            data-custom-class="body_text"
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              <span
                                                                                                style={{
                                                                                                  color:
                                                                                                    "",
                                                                                                  fontSize: 15,
                                                                                                }}
                                                                                              >
                                                                                                <span data-custom-class="body_text">
                                                                                                  <b className="block-component" />
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              <strong>
                                                                                                Will
                                                                                                your
                                                                                                information
                                                                                                be
                                                                                                shared
                                                                                                with
                                                                                                anyone
                                                                                                else?
                                                                                              </strong>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              We
                                                                                              may
                                                                                              disclose
                                                                                              your
                                                                                              personal
                                                                                              information
                                                                                              with
                                                                                              our
                                                                                              service
                                                                                              providers
                                                                                              pursuant
                                                                                              to
                                                                                              a
                                                                                              written
                                                                                              contract
                                                                                              between
                                                                                              us
                                                                                              and
                                                                                              each
                                                                                              service
                                                                                              provider.
                                                                                              Learn
                                                                                              more
                                                                                              about
                                                                                              how
                                                                                              we
                                                                                              disclose
                                                                                              personal
                                                                                              information
                                                                                              to
                                                                                              in
                                                                                              the
                                                                                              section,{" "}
                                                                                              <b className="block-component" />
                                                                                              "
                                                                                              <b className="statement-end-if-in-editor" />
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                        <a
                                                                                          data-custom-class="link"
                                                                                          href="#whoshare"
                                                                                          className="text-cyan-500 hover:underline"
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            >
                                                                                              WHEN
                                                                                              AND
                                                                                              WITH
                                                                                              WHOM
                                                                                              DO
                                                                                              WE
                                                                                              SHARE
                                                                                              YOUR
                                                                                              PERSONAL
                                                                                              INFORMATION?
                                                                                            </span>
                                                                                          </span>
                                                                                        </a>
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              <b className="block-component" />
                                                                                              "
                                                                                              <b className="statement-end-if-in-editor" />
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              We
                                                                                              may
                                                                                              use
                                                                                              your
                                                                                              personal
                                                                                              information
                                                                                              for
                                                                                              our
                                                                                              own
                                                                                              business
                                                                                              purposes,
                                                                                              such
                                                                                              as
                                                                                              for
                                                                                              undertaking
                                                                                              internal
                                                                                              research
                                                                                              for
                                                                                              technological
                                                                                              development
                                                                                              and
                                                                                              demonstration.
                                                                                              This
                                                                                              is
                                                                                              not
                                                                                              considered
                                                                                              to
                                                                                              be{" "}
                                                                                              <b className="block-component" />
                                                                                              "selling"
                                                                                              <b className="statement-end-if-in-editor" />{" "}
                                                                                              of
                                                                                              your
                                                                                              personal
                                                                                              information.
                                                                                              <span
                                                                                                style={{
                                                                                                  color:
                                                                                                    "",
                                                                                                  fontSize: 15,
                                                                                                }}
                                                                                              >
                                                                                                <span data-custom-class="body_text">
                                                                                                  <b className="block-component" />
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              We
                                                                                              have
                                                                                              not
                                                                                              disclosed,
                                                                                              sold,
                                                                                              or
                                                                                              shared
                                                                                              any
                                                                                              personal
                                                                                              information
                                                                                              to
                                                                                              third
                                                                                              parties
                                                                                              for
                                                                                              a
                                                                                              business
                                                                                              or
                                                                                              commercial
                                                                                              purpose
                                                                                              in
                                                                                              the
                                                                                              preceding
                                                                                              twelve
                                                                                              (12)
                                                                                              months.
                                                                                              We
                                                                                              <span
                                                                                                style={{
                                                                                                  color:
                                                                                                    "",
                                                                                                }}
                                                                                              >
                                                                                                &nbsp;
                                                                                              </span>
                                                                                              will
                                                                                              not
                                                                                              sell
                                                                                              or
                                                                                              share
                                                                                              personal
                                                                                              information
                                                                                              in
                                                                                              the
                                                                                              future
                                                                                              belonging
                                                                                              to
                                                                                              website
                                                                                              visitors,
                                                                                              users,
                                                                                              and
                                                                                              other
                                                                                              consumers.
                                                                                              <span
                                                                                                style={{
                                                                                                  color:
                                                                                                    "",
                                                                                                }}
                                                                                              >
                                                                                                <span data-custom-class="body_text">
                                                                                                  <span
                                                                                                    style={{
                                                                                                      color:
                                                                                                        "",
                                                                                                    }}
                                                                                                  >
                                                                                                    <span data-custom-class="body_text">
                                                                                                      <span
                                                                                                        style={{
                                                                                                          color:
                                                                                                            "",
                                                                                                        }}
                                                                                                      >
                                                                                                        <span data-custom-class="body_text">
                                                                                                          <b className="statement-end-if-in-editor" />
                                                                                                        </span>
                                                                                                      </span>
                                                                                                    </span>
                                                                                                  </span>
                                                                                                  <b className="block-component" />
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                        <span data-custom-class="body_text">
                                                                                          <span
                                                                                            style={{
                                                                                              color:
                                                                                                "rgb(0, 0, 0)",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              <b className="block-component" />
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <strong>
                                                                                            <span data-custom-class="heading_2">
                                                                                              Your
                                                                                              Rights
                                                                                            </span>
                                                                                          </strong>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <br />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            You
                                                                                            have
                                                                                            rights
                                                                                            under
                                                                                            certain
                                                                                            US
                                                                                            state
                                                                                            data
                                                                                            protection
                                                                                            laws.
                                                                                            However,
                                                                                            these
                                                                                            rights
                                                                                            are
                                                                                            not
                                                                                            absolute,
                                                                                            and
                                                                                            in
                                                                                            certain
                                                                                            cases,
                                                                                            we
                                                                                            may
                                                                                            decline
                                                                                            your
                                                                                            request
                                                                                            as
                                                                                            permitted
                                                                                            by
                                                                                            law.
                                                                                            These
                                                                                            rights
                                                                                            include:
                                                                                          </span>
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <strong>
                                                                                              Right
                                                                                              to
                                                                                              know
                                                                                            </strong>{" "}
                                                                                            whether
                                                                                            or
                                                                                            not
                                                                                            we
                                                                                            are
                                                                                            processing
                                                                                            your
                                                                                            personal
                                                                                            data
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <strong>
                                                                                              Right
                                                                                              to
                                                                                              access&nbsp;
                                                                                            </strong>
                                                                                            your
                                                                                            personal
                                                                                            data
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <strong>
                                                                                              Right
                                                                                              to
                                                                                              correct&nbsp;
                                                                                            </strong>
                                                                                            inaccuracies
                                                                                            in
                                                                                            your
                                                                                            personal
                                                                                            data
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <strong>
                                                                                              Right
                                                                                              to
                                                                                              request
                                                                                            </strong>{" "}
                                                                                            the
                                                                                            deletion
                                                                                            of
                                                                                            your
                                                                                            personal
                                                                                            data
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <strong>
                                                                                              Right
                                                                                              to
                                                                                              obtain
                                                                                              a
                                                                                              copy&nbsp;
                                                                                            </strong>
                                                                                            of
                                                                                            the
                                                                                            personal
                                                                                            data
                                                                                            you
                                                                                            previously
                                                                                            shared
                                                                                            with
                                                                                            us
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <strong>
                                                                                              Right
                                                                                              to
                                                                                              non-discrimination
                                                                                            </strong>{" "}
                                                                                            for
                                                                                            exercising
                                                                                            your
                                                                                            rights
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <strong>
                                                                                              Right
                                                                                              to
                                                                                              opt
                                                                                              out
                                                                                            </strong>{" "}
                                                                                            of
                                                                                            the
                                                                                            processing
                                                                                            of
                                                                                            your
                                                                                            personal
                                                                                            data
                                                                                            if
                                                                                            it
                                                                                            is
                                                                                            used
                                                                                            for
                                                                                            targeted
                                                                                            advertising
                                                                                            <b className="block-component" />{" "}
                                                                                            (or
                                                                                            sharing
                                                                                            as
                                                                                            defined
                                                                                            under
                                                                                            California’s
                                                                                            privacy
                                                                                            law)
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                            ,
                                                                                            the
                                                                                            sale
                                                                                            of
                                                                                            personal
                                                                                            data,
                                                                                            or
                                                                                            profiling
                                                                                            in
                                                                                            furtherance
                                                                                            of
                                                                                            decisions
                                                                                            that
                                                                                            produce
                                                                                            legal
                                                                                            or
                                                                                            similarly
                                                                                            significant
                                                                                            effects
                                                                                            (
                                                                                            <b className="block-component" />
                                                                                            "profiling"
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                            )
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            Depending
                                                                                            upon
                                                                                            the
                                                                                            state
                                                                                            where
                                                                                            you
                                                                                            live,
                                                                                            you
                                                                                            may
                                                                                            also
                                                                                            have
                                                                                            the
                                                                                            following
                                                                                            rights:
                                                                                          </span>
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            Right
                                                                                            to
                                                                                            access
                                                                                            the
                                                                                            categories
                                                                                            of
                                                                                            personal
                                                                                            data
                                                                                            being
                                                                                            processed
                                                                                            (as
                                                                                            permitted
                                                                                            by
                                                                                            applicable
                                                                                            law,
                                                                                            including
                                                                                            Minnesota’s
                                                                                            privacy
                                                                                            law)
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            Right
                                                                                            to
                                                                                            obtain
                                                                                            a
                                                                                            list
                                                                                            of
                                                                                            the
                                                                                            categories
                                                                                            of
                                                                                            third
                                                                                            parties
                                                                                            to
                                                                                            which
                                                                                            we
                                                                                            have
                                                                                            disclosed
                                                                                            personal
                                                                                            data
                                                                                            (as
                                                                                            permitted
                                                                                            by
                                                                                            applicable
                                                                                            law,
                                                                                            including
                                                                                            <b className="block-component" />{" "}
                                                                                            California's
                                                                                            and
                                                                                            Delaware's
                                                                                            <b className="else-block" />{" "}
                                                                                            privacy
                                                                                            law)
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            Right
                                                                                            to
                                                                                            obtain
                                                                                            a
                                                                                            list
                                                                                            of
                                                                                            specific
                                                                                            third
                                                                                            parties
                                                                                            to
                                                                                            which
                                                                                            we
                                                                                            have
                                                                                            disclosed
                                                                                            personal
                                                                                            data
                                                                                            (as
                                                                                            permitted
                                                                                            by
                                                                                            applicable
                                                                                            law,
                                                                                            including
                                                                                            <b className="block-component" />{" "}
                                                                                            Minnesota's
                                                                                            and
                                                                                            Oregon's
                                                                                            <b className="else-block" />{" "}
                                                                                            privacy
                                                                                            law)
                                                                                          </span>
                                                                                          <b className="statement-end-if-in-editor">
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            />
                                                                                          </b>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="block-component" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            Right
                                                                                            to
                                                                                            review,
                                                                                            understand,
                                                                                            question,
                                                                                            and
                                                                                            correct
                                                                                            how
                                                                                            personal
                                                                                            data
                                                                                            has
                                                                                            been
                                                                                            profiled
                                                                                            (as
                                                                                            permitted
                                                                                            by
                                                                                            applicable
                                                                                            law,
                                                                                            including
                                                                                            Minnesota’s
                                                                                            privacy
                                                                                            law)
                                                                                          </span>
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            Right
                                                                                            to
                                                                                            limit
                                                                                            use
                                                                                            and
                                                                                            disclosure
                                                                                            of
                                                                                            sensitive
                                                                                            personal
                                                                                            data
                                                                                            (as
                                                                                            permitted
                                                                                            by
                                                                                            applicable
                                                                                            law,
                                                                                            including
                                                                                            California’s
                                                                                            privacy
                                                                                            law)
                                                                                          </span>
                                                                                          <b className="statement-end-if-in-editor">
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            />
                                                                                          </b>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                      </div>
                                                                                      <ul className="list-disc mx-8">
                                                                                        <li
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            lineHeight:
                                                                                              "1.5",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            Right
                                                                                            to
                                                                                            opt
                                                                                            out
                                                                                            of
                                                                                            the
                                                                                            collection
                                                                                            of
                                                                                            sensitive
                                                                                            data
                                                                                            and
                                                                                            personal
                                                                                            data
                                                                                            collected
                                                                                            through
                                                                                            the
                                                                                            operation
                                                                                            of
                                                                                            a
                                                                                            voice
                                                                                            or
                                                                                            facial
                                                                                            recognition
                                                                                            feature
                                                                                            (as
                                                                                            permitted
                                                                                            by
                                                                                            applicable
                                                                                            law,
                                                                                            including
                                                                                            Florida’s
                                                                                            privacy
                                                                                            law)
                                                                                          </span>
                                                                                          <b className="statement-end-if-in-editor">
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            />
                                                                                          </b>
                                                                                        </li>
                                                                                      </ul>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="statement-end-if-in-editor" />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <strong>
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="heading_2">
                                                                                              How
                                                                                              to
                                                                                              Exercise
                                                                                              Your
                                                                                              Rights
                                                                                            </span>
                                                                                          </span>
                                                                                        </strong>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              To
                                                                                              exercise
                                                                                              these
                                                                                              rights,
                                                                                              you
                                                                                              can
                                                                                              contact
                                                                                              us{" "}
                                                                                              <b className="block-component" />
                                                                                              by
                                                                                              submitting
                                                                                              a&nbsp;
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                        <a
                                                                                          data-custom-class="link"
                                                                                          href="https://app.termly.io/dsar/ae423e99-f627-4e8a-bccb-728f87320370"
                                                                                          rel="noopener noreferrer"
                                                                                          target="_blank"
                                                                                          className="text-cyan-500 hover:underline"
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            >
                                                                                              data
                                                                                              subject
                                                                                              access
                                                                                              request
                                                                                            </span>
                                                                                          </span>
                                                                                        </a>
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              ,{" "}
                                                                                              <b className="block-component" />
                                                                                            </span>
                                                                                            <span
                                                                                              style={{
                                                                                                color:
                                                                                                  "",
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            >
                                                                                              <span data-custom-class="body_text">
                                                                                                <span
                                                                                                  style={{
                                                                                                    color:
                                                                                                      "",
                                                                                                    fontSize: 15,
                                                                                                  }}
                                                                                                >
                                                                                                  <span data-custom-class="body_text">
                                                                                                    <b className="block-component" />
                                                                                                    by
                                                                                                    emailing
                                                                                                    us
                                                                                                    at{" "}
                                                                                                    <b className="question">
                                                                                                      <a
                                                                                                        target="_blank"
                                                                                                        data-custom-class="link"
                                                                                                        href="mailto:adalundhe@lundhe.audio"
                                                                                                        className="text-cyan-500 hover:underline"
                                                                                                      >
                                                                                                        adalundhe@lundhe.audio
                                                                                                      </a>
                                                                                                    </b>

                                                                                                    ,{" "}
                                                                                                    <b className="statement-end-if-in-editor" />
                                                                                                    <b className="block-component" />
                                                                                                  </span>
                                                                                                  <span data-custom-class="body_text">
                                                                                                    <b className="block-component" />
                                                                                                    by
                                                                                                    visiting{" "}
                                                                                                    <span>
                                                                                                      <b className="question">
                                                                                                        <a
                                                                                                          target="_blank"
                                                                                                          data-custom-class="link"
                                                                                                          href="http://www.lundhe.audio/contact"
                                                                                                          className="text-cyan-500 hover:underline"
                                                                                                        >
                                                                                                          www.lundhe.audio/contact
                                                                                                        </a>
                                                                                                      </b>
                                                                                                    </span>

                                                                                                    ,{" "}
                                                                                                    <b className="statement-end-if-in-editor" />
                                                                                                    <b className="block-component">
                                                                                                      <span data-custom-class="body_text">
                                                                                                        <b className="block-component" />
                                                                                                      </span>
                                                                                                    </b>
                                                                                                  </span>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                        <span data-custom-class="body_text">
                                                                                          or
                                                                                          by
                                                                                          referring
                                                                                          to
                                                                                          the
                                                                                          contact
                                                                                          details
                                                                                          at
                                                                                          the
                                                                                          bottom
                                                                                          of
                                                                                          this
                                                                                          document.
                                                                                        </span>
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            Under
                                                                                            certain
                                                                                            US
                                                                                            state
                                                                                            data
                                                                                            protection
                                                                                            laws,
                                                                                            you
                                                                                            can
                                                                                            designate
                                                                                            an{" "}
                                                                                            <b className="block-component" />
                                                                                            authorized
                                                                                            <b className="statement-end-if-in-editor" />{" "}
                                                                                            agent
                                                                                            to
                                                                                            make
                                                                                            a
                                                                                            request
                                                                                            on
                                                                                            your
                                                                                            behalf.
                                                                                            We
                                                                                            may
                                                                                            deny
                                                                                            a
                                                                                            request
                                                                                            from
                                                                                            an{" "}
                                                                                            <b className="block-component" />
                                                                                            authorized
                                                                                            <b className="statement-end-if-in-editor" />{" "}
                                                                                            agent
                                                                                            that
                                                                                            does
                                                                                            not
                                                                                            submit
                                                                                            proof
                                                                                            that
                                                                                            they
                                                                                            have
                                                                                            been
                                                                                            validly{" "}
                                                                                            <b className="block-component" />
                                                                                            authorized
                                                                                            <b className="statement-end-if-in-editor" />{" "}
                                                                                            to
                                                                                            act
                                                                                            on
                                                                                            your
                                                                                            behalf
                                                                                            in
                                                                                            accordance
                                                                                            with
                                                                                            applicable
                                                                                            laws.
                                                                                          </span>
                                                                                          <br />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <strong>
                                                                                            <span data-custom-class="heading_2">
                                                                                              Request
                                                                                              Verification
                                                                                            </span>
                                                                                          </strong>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <br />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            Upon
                                                                                            receiving
                                                                                            your
                                                                                            request,
                                                                                            we
                                                                                            will
                                                                                            need
                                                                                            to
                                                                                            verify
                                                                                            your
                                                                                            identity
                                                                                            to
                                                                                            determine
                                                                                            you
                                                                                            are
                                                                                            the
                                                                                            same
                                                                                            person
                                                                                            about
                                                                                            whom
                                                                                            we
                                                                                            have
                                                                                            the
                                                                                            information
                                                                                            in
                                                                                            our
                                                                                            system.
                                                                                            We
                                                                                            will
                                                                                            only
                                                                                            use
                                                                                            personal
                                                                                            information
                                                                                            provided
                                                                                            in
                                                                                            your
                                                                                            request
                                                                                            to
                                                                                            verify
                                                                                            your
                                                                                            identity
                                                                                            or
                                                                                            authority
                                                                                            to
                                                                                            make
                                                                                            the
                                                                                            request.
                                                                                            However,
                                                                                            if
                                                                                            we
                                                                                            cannot
                                                                                            verify
                                                                                            your
                                                                                            identity
                                                                                            from
                                                                                            the
                                                                                            information
                                                                                            already
                                                                                            maintained
                                                                                            by
                                                                                            us,
                                                                                            we
                                                                                            may
                                                                                            request
                                                                                            that
                                                                                            you
                                                                                            provide
                                                                                            additional
                                                                                            information
                                                                                            for
                                                                                            the
                                                                                            purposes
                                                                                            of
                                                                                            verifying
                                                                                            your
                                                                                            identity
                                                                                            and
                                                                                            for
                                                                                            security
                                                                                            or
                                                                                            fraud-prevention
                                                                                            purposes.
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <br />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            If
                                                                                            you
                                                                                            submit
                                                                                            the
                                                                                            request
                                                                                            through
                                                                                            an{" "}
                                                                                            <b className="block-component" />
                                                                                            authorized
                                                                                            <b className="statement-end-if-in-editor" />{" "}
                                                                                            agent,
                                                                                            we
                                                                                            may
                                                                                            need
                                                                                            to
                                                                                            collect
                                                                                            additional
                                                                                            information
                                                                                            to
                                                                                            verify
                                                                                            your
                                                                                            identity
                                                                                            before
                                                                                            processing
                                                                                            your
                                                                                            request
                                                                                            and
                                                                                            the
                                                                                            agent
                                                                                            will
                                                                                            need
                                                                                            to
                                                                                            provide
                                                                                            a
                                                                                            written
                                                                                            and
                                                                                            signed
                                                                                            permission
                                                                                            from
                                                                                            you
                                                                                            to
                                                                                            submit
                                                                                            such
                                                                                            request
                                                                                            on
                                                                                            your
                                                                                            behalf.
                                                                                          </span>
                                                                                        </span>
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <br />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="heading_2">
                                                                                            <strong>
                                                                                              Appeals
                                                                                            </strong>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <br />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            Under
                                                                                            certain
                                                                                            US
                                                                                            state
                                                                                            data
                                                                                            protection
                                                                                            laws,
                                                                                            if
                                                                                            we
                                                                                            decline
                                                                                            to
                                                                                            take
                                                                                            action
                                                                                            regarding
                                                                                            your
                                                                                            request,
                                                                                            you
                                                                                            may
                                                                                            appeal
                                                                                            our
                                                                                            decision
                                                                                            by
                                                                                            emailing
                                                                                            us
                                                                                            at{" "}
                                                                                            <b className="block-component" />
                                                                                            <b className="question">
                                                                                              <a
                                                                                                target="_blank"
                                                                                                data-custom-class="link"
                                                                                                href="mailto:adalundhe@lundhe.audio"
                                                                                                className="text-cyan-500 hover:underline"
                                                                                              >
                                                                                                adalundhe@lundhe.audio
                                                                                              </a>
                                                                                            </b>
                                                                                            <b className="else-block" />

                                                                                            .
                                                                                            We
                                                                                            will
                                                                                            inform
                                                                                            you
                                                                                            in
                                                                                            writing
                                                                                            of
                                                                                            any
                                                                                            action
                                                                                            taken
                                                                                            or
                                                                                            not
                                                                                            taken
                                                                                            in
                                                                                            response
                                                                                            to
                                                                                            the
                                                                                            appeal,
                                                                                            including
                                                                                            a
                                                                                            written
                                                                                            explanation
                                                                                            of
                                                                                            the
                                                                                            reasons
                                                                                            for
                                                                                            the
                                                                                            decisions.
                                                                                            If
                                                                                            your
                                                                                            appeal
                                                                                            is
                                                                                            denied,
                                                                                            you
                                                                                            may
                                                                                            submit
                                                                                            a
                                                                                            complaint
                                                                                            to
                                                                                            your
                                                                                            state
                                                                                            attorney
                                                                                            general.
                                                                                          </span>
                                                                                          <b className="statement-end-if-in-editor" />
                                                                                        </span>
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                color:
                                                                                                  "",
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            >
                                                                                              <span data-custom-class="body_text">
                                                                                                <b className="block-component">
                                                                                                  <b className="block-component" />
                                                                                                </b>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <br />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <strong>
                                                                                            <span data-custom-class="heading_2">
                                                                                              California{" "}
                                                                                              <b className="block-component" />
                                                                                              "Shine
                                                                                              The
                                                                                              Light"
                                                                                              <b className="statement-end-if-in-editor" />{" "}
                                                                                              Law
                                                                                            </span>
                                                                                          </strong>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <br />
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            California
                                                                                            Civil
                                                                                            Code
                                                                                            Section
                                                                                            1798.83,
                                                                                            also
                                                                                            known
                                                                                            as
                                                                                            the{" "}
                                                                                            <b className="block-component" />
                                                                                            "Shine
                                                                                            The
                                                                                            Light"
                                                                                            <b className="statement-end-if-in-editor" />{" "}
                                                                                            law,
                                                                                            permits
                                                                                            our
                                                                                            users
                                                                                            who
                                                                                            are
                                                                                            California
                                                                                            residents
                                                                                            to
                                                                                            request
                                                                                            and
                                                                                            obtain
                                                                                            from
                                                                                            us,
                                                                                            once
                                                                                            a
                                                                                            year
                                                                                            and
                                                                                            free
                                                                                            of
                                                                                            charge,
                                                                                            information
                                                                                            about
                                                                                            categories
                                                                                            of
                                                                                            personal
                                                                                            information
                                                                                            (if
                                                                                            any)
                                                                                            we
                                                                                            disclosed
                                                                                            to
                                                                                            third
                                                                                            parties
                                                                                            for
                                                                                            direct
                                                                                            marketing
                                                                                            purposes
                                                                                            and
                                                                                            the
                                                                                            names
                                                                                            and
                                                                                            addresses
                                                                                            of
                                                                                            all
                                                                                            third
                                                                                            parties
                                                                                            with
                                                                                            which
                                                                                            we
                                                                                            shared
                                                                                            personal
                                                                                            information
                                                                                            in
                                                                                            the
                                                                                            immediately
                                                                                            preceding
                                                                                            calendar
                                                                                            year.
                                                                                            If
                                                                                            you
                                                                                            are
                                                                                            a
                                                                                            California
                                                                                            resident
                                                                                            and
                                                                                            would
                                                                                            like
                                                                                            to
                                                                                            make
                                                                                            such
                                                                                            a
                                                                                            request,
                                                                                            please
                                                                                            submit
                                                                                            your
                                                                                            request
                                                                                            in
                                                                                            writing
                                                                                            to
                                                                                            us
                                                                                            by
                                                                                            using
                                                                                            the
                                                                                            contact
                                                                                            details
                                                                                            provided
                                                                                            in
                                                                                            the
                                                                                            section{" "}
                                                                                            <b className="block-component" />
                                                                                            "
                                                                                            <b className="statement-end-if-in-editor" />
                                                                                          </span>
                                                                                        </span>
                                                                                        <span data-custom-class="body_text">
                                                                                          <a
                                                                                            data-custom-class="link"
                                                                                            href="#contact"
                                                                                            className="text-cyan-500 hover:underline"
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            >
                                                                                              HOW
                                                                                              CAN
                                                                                              YOU
                                                                                              CONTACT
                                                                                              US
                                                                                              ABOUT
                                                                                              THIS
                                                                                              NOTICE?
                                                                                            </span>
                                                                                          </a>
                                                                                        </span>
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            <b className="block-component" />

                                                                                            "
                                                                                          </span>
                                                                                          <b className="statement-end-if-in-editor">
                                                                                            <span data-custom-class="body_text" />
                                                                                          </b>
                                                                                        </span>
                                                                                        <b className="statement-end-if-in-editor">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                color:
                                                                                                  "",
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            >
                                                                                              <span data-custom-class="body_text">
                                                                                                <span
                                                                                                  style={{
                                                                                                    color:
                                                                                                      "",
                                                                                                    fontSize: 15,
                                                                                                  }}
                                                                                                >
                                                                                                  <span data-custom-class="body_text">
                                                                                                    <span
                                                                                                      style={{
                                                                                                        color:
                                                                                                          "",
                                                                                                        fontSize: 15,
                                                                                                      }}
                                                                                                    >
                                                                                                      <span data-custom-class="body_text">
                                                                                                        <span
                                                                                                          style={{
                                                                                                            color:
                                                                                                              "",
                                                                                                            fontSize: 15,
                                                                                                          }}
                                                                                                        >
                                                                                                          <span data-custom-class="body_text">
                                                                                                            <b className="statement-end-if-in-editor">
                                                                                                              <span data-custom-class="body_text">
                                                                                                                <span
                                                                                                                  style={{
                                                                                                                    fontSize: 15,
                                                                                                                    color:
                                                                                                                      "",
                                                                                                                  }}
                                                                                                                >
                                                                                                                  <span
                                                                                                                    style={{
                                                                                                                      fontSize: 15,
                                                                                                                      color:
                                                                                                                        "",
                                                                                                                    }}
                                                                                                                  >
                                                                                                                    <span
                                                                                                                      style={{
                                                                                                                        color:
                                                                                                                          "",
                                                                                                                        fontSize: 15,
                                                                                                                      }}
                                                                                                                    >
                                                                                                                      <span data-custom-class="body_text">
                                                                                                                        <span
                                                                                                                          style={{
                                                                                                                            color:
                                                                                                                              "",
                                                                                                                            fontSize: 15,
                                                                                                                          }}
                                                                                                                        >
                                                                                                                          <span data-custom-class="body_text">
                                                                                                                            <span
                                                                                                                              style={{
                                                                                                                                color:
                                                                                                                                  "",
                                                                                                                                fontSize: 15,
                                                                                                                              }}
                                                                                                                            >
                                                                                                                              <span data-custom-class="body_text">
                                                                                                                                <span
                                                                                                                                  style={{
                                                                                                                                    color:
                                                                                                                                      "",
                                                                                                                                    fontSize: 15,
                                                                                                                                  }}
                                                                                                                                >
                                                                                                                                  <span data-custom-class="body_text">
                                                                                                                                    <b className="statement-end-if-in-editor">
                                                                                                                                      <b className="statement-end-if-in-editor" />
                                                                                                                                    </b>
                                                                                                                                  </span>
                                                                                                                                </span>
                                                                                                                              </span>
                                                                                                                            </span>
                                                                                                                          </span>
                                                                                                                        </span>
                                                                                                                      </span>
                                                                                                                    </span>
                                                                                                                  </span>
                                                                                                                </span>
                                                                                                              </span>
                                                                                                            </b>
                                                                                                          </span>
                                                                                                        </span>
                                                                                                      </span>
                                                                                                    </span>
                                                                                                  </span>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          />
                                                                                        </b>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        id="policyupdates"
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            color:
                                                                                              "rgb(127, 127, 127)",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              color:
                                                                                                "",
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                                color:
                                                                                                  "",
                                                                                              }}
                                                                                            >
                                                                                              <span
                                                                                                style={{
                                                                                                  fontSize: 15,
                                                                                                  color:
                                                                                                    "",
                                                                                                }}
                                                                                              >
                                                                                                <span
                                                                                                  id="control"
                                                                                                  style={{
                                                                                                    color:
                                                                                                      "rgb(0, 0, 0)",
                                                                                                  }}
                                                                                                >
                                                                                                  <strong>
                                                                                                    <span data-custom-class="heading_1">
                                                                                                      10.
                                                                                                      DO
                                                                                                      WE
                                                                                                      MAKE
                                                                                                      UPDATES
                                                                                                      TO
                                                                                                      THIS
                                                                                                      NOTICE?
                                                                                                    </span>
                                                                                                  </strong>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              <em>
                                                                                                <strong>
                                                                                                  In
                                                                                                  Short:&nbsp;
                                                                                                </strong>
                                                                                                Yes,
                                                                                                we
                                                                                                will
                                                                                                update
                                                                                                this
                                                                                                notice
                                                                                                as
                                                                                                necessary
                                                                                                to
                                                                                                stay
                                                                                                compliant
                                                                                                with
                                                                                                relevant
                                                                                                laws.
                                                                                              </em>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              We
                                                                                              may
                                                                                              update
                                                                                              this
                                                                                              Privacy
                                                                                              Notice
                                                                                              from
                                                                                              time
                                                                                              to
                                                                                              time.
                                                                                              The
                                                                                              updated
                                                                                              version
                                                                                              will
                                                                                              be
                                                                                              indicated
                                                                                              by
                                                                                              an
                                                                                              updated{" "}
                                                                                              <b className="block-component" />
                                                                                              "Revised"
                                                                                              <b className="statement-end-if-in-editor" />{" "}
                                                                                              date
                                                                                              at
                                                                                              the
                                                                                              top
                                                                                              of
                                                                                              this
                                                                                              Privacy
                                                                                              Notice.
                                                                                              If
                                                                                              we
                                                                                              make
                                                                                              material
                                                                                              changes
                                                                                              to
                                                                                              this
                                                                                              Privacy
                                                                                              Notice,
                                                                                              we
                                                                                              may
                                                                                              notify
                                                                                              you
                                                                                              either
                                                                                              by
                                                                                              prominently
                                                                                              posting
                                                                                              a
                                                                                              notice
                                                                                              of
                                                                                              such
                                                                                              changes
                                                                                              or
                                                                                              by
                                                                                              directly
                                                                                              sending
                                                                                              you
                                                                                              a
                                                                                              notification.
                                                                                              We
                                                                                              encourage
                                                                                              you
                                                                                              to
                                                                                              review
                                                                                              this
                                                                                              Privacy
                                                                                              Notice
                                                                                              frequently
                                                                                              to
                                                                                              be
                                                                                              informed
                                                                                              of
                                                                                              how
                                                                                              we
                                                                                              are
                                                                                              protecting
                                                                                              your
                                                                                              information.
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        id="contact"
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            color:
                                                                                              "rgb(127, 127, 127)",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              color:
                                                                                                "",
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                                color:
                                                                                                  "",
                                                                                              }}
                                                                                            >
                                                                                              <span
                                                                                                style={{
                                                                                                  fontSize: 15,
                                                                                                  color:
                                                                                                    "",
                                                                                                }}
                                                                                              >
                                                                                                <span
                                                                                                  id="control"
                                                                                                  style={{
                                                                                                    color:
                                                                                                      "rgb(0, 0, 0)",
                                                                                                  }}
                                                                                                >
                                                                                                  <strong>
                                                                                                    <span data-custom-class="heading_1">
                                                                                                      11.
                                                                                                      HOW
                                                                                                      CAN
                                                                                                      YOU
                                                                                                      CONTACT
                                                                                                      US
                                                                                                      ABOUT
                                                                                                      THIS
                                                                                                      NOTICE?
                                                                                                    </span>
                                                                                                  </strong>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              If
                                                                                              you
                                                                                              have
                                                                                              questions
                                                                                              or
                                                                                              comments
                                                                                              about
                                                                                              this
                                                                                              notice,
                                                                                              you
                                                                                              may{" "}
                                                                                              <span
                                                                                                style={{
                                                                                                  fontSize: 15,
                                                                                                }}
                                                                                              >
                                                                                                <span data-custom-class="body_text">
                                                                                                  <b className="block-component">
                                                                                                    <b className="block-component" />
                                                                                                  </b>
                                                                                                  email
                                                                                                  us
                                                                                                  at{" "}
                                                                                                  <b className="question">
                                                                                                    <a
                                                                                                      target="_blank"
                                                                                                      data-custom-class="link"
                                                                                                      href="mailto:adalundhe@lundhe.audio"
                                                                                                      className="text-cyan-500 hover:underline"
                                                                                                    >
                                                                                                      adalundhe@lundhe.audio
                                                                                                    </a>{" "}
                                                                                                    or&nbsp;
                                                                                                  </b>
                                                                                                  <b className="statement-end-if-in-editor">
                                                                                                    <b className="block-component" />
                                                                                                  </b>
                                                                                                </span>
                                                                                              </span>
                                                                                              <span
                                                                                                style={{
                                                                                                  fontSize: 15,
                                                                                                  color:
                                                                                                    "",
                                                                                                }}
                                                                                              >
                                                                                                <span
                                                                                                  style={{
                                                                                                    fontSize: 15,
                                                                                                    color:
                                                                                                      "",
                                                                                                  }}
                                                                                                >
                                                                                                  <span data-custom-class="body_text">
                                                                                                    contact
                                                                                                    us
                                                                                                    by
                                                                                                    post
                                                                                                    at:
                                                                                                  </span>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              <span
                                                                                                style={{
                                                                                                  fontSize: 15,
                                                                                                }}
                                                                                              >
                                                                                                <span
                                                                                                  style={{
                                                                                                    color:
                                                                                                      "",
                                                                                                  }}
                                                                                                >
                                                                                                  <span
                                                                                                    style={{
                                                                                                      color:
                                                                                                        "",
                                                                                                    }}
                                                                                                  >
                                                                                                    <span data-custom-class="body_text">
                                                                                                      <b className="question">
                                                                                                        Lundhe
                                                                                                        Audio
                                                                                                        LLC
                                                                                                      </b>
                                                                                                    </span>
                                                                                                  </span>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                            <span data-custom-class="body_text">
                                                                                              <span
                                                                                                style={{
                                                                                                  color:
                                                                                                    "",
                                                                                                }}
                                                                                              >
                                                                                                <span data-custom-class="body_text">
                                                                                                  <b className="block-component" />
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            <b className="question">
                                                                                              5705
                                                                                              Abby
                                                                                              Ann
                                                                                              Ln
                                                                                            </b>
                                                                                            <span
                                                                                              style={{
                                                                                                color:
                                                                                                  "",
                                                                                              }}
                                                                                            >
                                                                                              <span
                                                                                                style={{
                                                                                                  fontSize: 15,
                                                                                                }}
                                                                                              >
                                                                                                <b className="block-component" />
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <span data-custom-class="body_text">
                                                                                            <b className="question">
                                                                                              Austin
                                                                                            </b>
                                                                                            <span
                                                                                              style={{
                                                                                                color:
                                                                                                  "",
                                                                                              }}
                                                                                            >
                                                                                              <span
                                                                                                style={{
                                                                                                  fontSize: 15,
                                                                                                }}
                                                                                              >
                                                                                                <b className="block-component" />
                                                                                                <b className="block-component" />

                                                                                                ,{" "}
                                                                                                <b className="question">
                                                                                                  TX
                                                                                                </b>
                                                                                                <b className="statement-end-if-in-editor" />
                                                                                                <b className="block-component" />
                                                                                                <b className="block-component" />{" "}
                                                                                                <b className="question">
                                                                                                  78747
                                                                                                </b>
                                                                                                <b className="statement-end-if-in-editor" />
                                                                                                <b className="block-component" />
                                                                                                <b className="block-component" />
                                                                                                <b className="block-component" />
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          data-custom-class="body_text"
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                          }}
                                                                                        >
                                                                                          <b className="question">
                                                                                            United
                                                                                            States
                                                                                            <span data-custom-class="body_text">
                                                                                              <span
                                                                                                style={{
                                                                                                  color:
                                                                                                    "",
                                                                                                }}
                                                                                              >
                                                                                                <span
                                                                                                  style={{
                                                                                                    fontSize: 15,
                                                                                                  }}
                                                                                                >
                                                                                                  <span data-custom-class="body_text">
                                                                                                    <span
                                                                                                      style={{
                                                                                                        color:
                                                                                                          "",
                                                                                                      }}
                                                                                                    >
                                                                                                      <span
                                                                                                        style={{
                                                                                                          fontSize: 15,
                                                                                                        }}
                                                                                                      >
                                                                                                        <span data-custom-class="body_text">
                                                                                                          <span
                                                                                                            style={{
                                                                                                              color:
                                                                                                                "",
                                                                                                            }}
                                                                                                          >
                                                                                                            <span
                                                                                                              style={{
                                                                                                                fontSize: 15,
                                                                                                              }}
                                                                                                            >
                                                                                                              <b className="statement-end-if-in-editor" />
                                                                                                            </span>
                                                                                                          </span>
                                                                                                        </span>
                                                                                                      </span>
                                                                                                    </span>
                                                                                                  </span>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </b>
                                                                                        </span>
                                                                                        <b className="block-component">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text" />
                                                                                          </span>
                                                                                        </b>
                                                                                        <b className="statement-end-if-in-editor" />
                                                                                        <span data-custom-class="body_text">
                                                                                          <span
                                                                                            style={{
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            >
                                                                                              <b className="statement-end-if-in-editor" />
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                        <span data-custom-class="body_text">
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              <span
                                                                                                style={{
                                                                                                  color:
                                                                                                    "",
                                                                                                }}
                                                                                              >
                                                                                                <b className="statement-end-if-in-editor">
                                                                                                  <span
                                                                                                    style={{
                                                                                                      color:
                                                                                                        "",
                                                                                                    }}
                                                                                                  >
                                                                                                    <span
                                                                                                      style={{
                                                                                                        fontSize: 15,
                                                                                                      }}
                                                                                                    >
                                                                                                      <span data-custom-class="body_text">
                                                                                                        <b className="block-component">
                                                                                                          <b className="block-component" />
                                                                                                        </b>
                                                                                                      </span>
                                                                                                    </span>
                                                                                                  </span>
                                                                                                </b>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                          <b className="block-component">
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                              }}
                                                                                            />
                                                                                          </b>
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              <span
                                                                                                style={{
                                                                                                  color:
                                                                                                    "",
                                                                                                  fontSize: 15,
                                                                                                }}
                                                                                              >
                                                                                                <span
                                                                                                  style={{
                                                                                                    fontSize: 15,
                                                                                                  }}
                                                                                                >
                                                                                                  <span data-custom-class="body_text">
                                                                                                    <b className="statement-end-if-in-editor">
                                                                                                      <b className="block-component" />
                                                                                                    </b>
                                                                                                  </span>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        id="request"
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            color:
                                                                                              "rgb(127, 127, 127)",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              color:
                                                                                                "",
                                                                                              fontSize: 15,
                                                                                            }}
                                                                                          >
                                                                                            <span
                                                                                              style={{
                                                                                                fontSize: 15,
                                                                                                color:
                                                                                                  "",
                                                                                              }}
                                                                                            >
                                                                                              <span
                                                                                                style={{
                                                                                                  fontSize: 15,
                                                                                                  color:
                                                                                                    "",
                                                                                                }}
                                                                                              >
                                                                                                <span
                                                                                                  id="control"
                                                                                                  style={{
                                                                                                    color:
                                                                                                      "rgb(0, 0, 0)",
                                                                                                  }}
                                                                                                >
                                                                                                  <strong>
                                                                                                    <span data-custom-class="heading_1">
                                                                                                      12.
                                                                                                      HOW
                                                                                                      CAN
                                                                                                      YOU
                                                                                                      REVIEW,
                                                                                                      UPDATE,
                                                                                                      OR
                                                                                                      DELETE
                                                                                                      THE
                                                                                                      DATA
                                                                                                      WE
                                                                                                      COLLECT
                                                                                                      FROM
                                                                                                      YOU?
                                                                                                    </span>
                                                                                                  </strong>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                          </span>
                                                                                        </span>
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <br />
                                                                                      </div>
                                                                                      <div
                                                                                        style={{
                                                                                          lineHeight:
                                                                                            "1.5",
                                                                                        }}
                                                                                      >
                                                                                        <span
                                                                                          style={{
                                                                                            fontSize: 15,
                                                                                            color:
                                                                                              "",
                                                                                          }}
                                                                                        >
                                                                                          <span
                                                                                            style={{
                                                                                              fontSize: 15,
                                                                                              color:
                                                                                                "",
                                                                                            }}
                                                                                          >
                                                                                            <span data-custom-class="body_text">
                                                                                              <b className="block-component" />
                                                                                              Based
                                                                                              on
                                                                                              the
                                                                                              applicable
                                                                                              laws
                                                                                              of
                                                                                              your
                                                                                              country
                                                                                              <b className="block-component" />{" "}
                                                                                              or
                                                                                              state
                                                                                              of
                                                                                              residence
                                                                                              in
                                                                                              the
                                                                                              US
                                                                                              <b className="statement-end-if-in-editor" />

                                                                                              ,
                                                                                              you
                                                                                              may
                                                                                              <b className="else-block">
                                                                                                <b className="block-component">
                                                                                                  {" "}
                                                                                                  have
                                                                                                  the
                                                                                                  right
                                                                                                  to
                                                                                                  request
                                                                                                  access
                                                                                                  to
                                                                                                  the
                                                                                                  personal
                                                                                                  information
                                                                                                  we
                                                                                                  collect
                                                                                                  from
                                                                                                  you,
                                                                                                  details
                                                                                                  about
                                                                                                  how
                                                                                                  we
                                                                                                  have
                                                                                                  processed
                                                                                                  it,
                                                                                                  correct
                                                                                                  inaccuracies,
                                                                                                  or
                                                                                                  delete
                                                                                                  your
                                                                                                  personal
                                                                                                  information.
                                                                                                  You
                                                                                                  may
                                                                                                  also
                                                                                                  have
                                                                                                  the
                                                                                                  right
                                                                                                  to{" "}
                                                                                                  <b className="block-component" />
                                                                                                  withdraw
                                                                                                  your
                                                                                                  consent
                                                                                                  to
                                                                                                  our
                                                                                                  processing
                                                                                                  of
                                                                                                  your
                                                                                                  personal
                                                                                                  information.
                                                                                                  These
                                                                                                  rights
                                                                                                  may
                                                                                                  be
                                                                                                  limited
                                                                                                  in
                                                                                                  some
                                                                                                  circumstances
                                                                                                  by
                                                                                                  applicable
                                                                                                  law.
                                                                                                  To
                                                                                                  request
                                                                                                  to
                                                                                                  review,
                                                                                                  update,
                                                                                                  or
                                                                                                  delete
                                                                                                  your
                                                                                                  personal
                                                                                                  information,
                                                                                                  please{" "}
                                                                                                  <b className="block-component" />
                                                                                                  fill
                                                                                                  out
                                                                                                  and
                                                                                                  submit
                                                                                                  a&nbsp;
                                                                                                </b>
                                                                                              </b>
                                                                                            </span>
                                                                                            <span
                                                                                              style={{
                                                                                                color:
                                                                                                  "rgb(0, 58, 250)",
                                                                                              }}
                                                                                            >
                                                                                              <span data-custom-class="body_text">
                                                                                                <span
                                                                                                  style={{
                                                                                                    fontSize: 15,
                                                                                                  }}
                                                                                                >
                                                                                                  <a
                                                                                                    data-custom-class="link"
                                                                                                    href="https://app.termly.io/dsar/ae423e99-f627-4e8a-bccb-728f87320370"
                                                                                                    rel="noopener noreferrer"
                                                                                                    target="_blank"
                                                                                                    className="text-cyan-500 hover:underline"
                                                                                                  >
                                                                                                    data
                                                                                                    subject
                                                                                                    access
                                                                                                    request
                                                                                                  </a>
                                                                                                </span>
                                                                                              </span>
                                                                                            </span>
                                                                                            <b className="block-component">
                                                                                              <span data-custom-class="body_text" />
                                                                                            </b>
                                                                                          </span>
                                                                                        </span>
                                                                                        <span data-custom-class="body_text">
                                                                                          .
                                                                                        </span>
                                                                                      </div>
                                                                                      <style
                                                                                        dangerouslySetInnerHTML={{
                                                                                          __html:
                                                                                            "\n      ul {\n        list-style-type: square;\n      }\n      ul > li > ul {\n        list-style-type: circle;\n      }\n      ul > li > ul > li > ul {\n        list-style-type: square;\n      }\n      ol li {\n        font-family: Arial ;\n      }\n    ",
                                                                                        }}
                                                                                      />
                                                                                    </div>
                                                                                  </div>
                                                                                </div>
                                                                              </div>
                                                                            </div>
                                                                          </div>
                                                                        </div>
                                                                      </div>
                                                                    </div>
                                                                  </div>
                                                                </div>
                                                              </div>
                                                            </div>
                                                          </div>
                                                        </div>
                                                      </div>
                                                    </div>
                                                  </div>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </CardContent>
        </Card>
      </Layout>
    </>
  );
}
