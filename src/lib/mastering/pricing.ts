import type { Song, AddOns, DeliveryOptions } from "@/components/studio-calculator"
...
export function hasMixdownBundle(songId: string, deliveryOptions: DeliveryOptions): boolean {
  return deliveryOptions.highResolution.includes(songId) && deliveryOptions.filmMixdown.includes(songId)
}
...
export function getProductionDealType(addOns: AddOns): ProductionDealType {
  const hasVocal = addOns.vocalProduction.length > 0
  const hasDrum = addOns.drumReplacement.length > 0
  const hasGuitar = addOns.guitarReamp.length > 0
...
export function calculateDeliveryCost(songs: Song[], deliveryOptions: DeliveryOptions): number {
  ...
  const highResCount = deliveryOptions.highResolution.length
  const filmMixdownCount = deliveryOptions.filmMixdown.length
  const extendedArchivalCount = deliveryOptions.extendedArchival.length
  const mixedStemsCount = deliveryOptions.mixedStems.length
  const rushDeliveryCount = deliveryOptions.rushDelivery.length
  ...
  for (const songId of deliveryOptions.mixedStems) {
  ...
  for (const songId of deliveryOptions.rushDelivery) {
    const song = songs.find((s) => s.id === songId)
    if (song) {
...
export function calculateTotalPrice(songs: Song[], addOns: AddOns, deliveryOptions: DeliveryOptions): number {
  ...
  const vocalCount = addOns.vocalProduction.length
  const drumCount = addOns.drumReplacement.length
  const guitarCount = addOns.guitarReamp.length
...
export function buildQuoteData(songs: Song[], addOns: AddOns, deliveryOptions: DeliveryOptions): QuoteData {
  ...
  const songDetails: SongPricingDetail[] = songs.map((song) => {
    ...
    const hasVocalProduction = addOns.vocalProduction.includes(song.id)
    const hasDrumReplacement = addOns.drumReplacement.includes(song.id)
    const hasGuitarReamp = addOns.guitarReamp.includes(song.id)

    const hasHighResolution = deliveryOptions.highResolution.includes(song.id)
    const hasFilmMixdown = deliveryOptions.filmMixdown.includes(song.id)
    const hasMixedStems = deliveryOptions.mixedStems.includes(song.id)
    const hasExtendedArchival = deliveryOptions.extendedArchival.includes(song.id)
    const hasRushDelivery = deliveryOptions.rushDelivery.includes(song.id)
